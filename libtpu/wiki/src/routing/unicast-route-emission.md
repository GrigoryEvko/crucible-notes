...
> **NOTE —** the VC-balance arithmetic of `GetVcBalanceUsage @0x1fbdb4c0` + `CreateVcBalanceThreshold @0x1fbd8320` + `SetChannelMerges @0x1fbda1e0` is traced to its three inputs but **not reduced to a closed VC-assignment rule** (LOW confidence on the exact `{0,1,2}` mapping). The inputs are `{CrossesDateline, Direction::IsSame, balance-counter}`; `vc=0` is same-VC, `vc=1` is the dateline-bump (deadlock break), `vc=2` is the balance-shifted VC.
