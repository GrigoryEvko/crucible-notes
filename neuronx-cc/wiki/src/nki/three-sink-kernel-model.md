...
No `klir_binary`, no `kernel_format`, no `nki_binary_version_identifier` — those are IT56-only fields (§4). The setter strings above are present in every IT54 body; see [BIR Kernel-Inst Nodes](../bir/codegen-helpers-kernel-provision.md) (Part 7, planned).
...
*Cross-reference: [BIR Kernel-Inst Nodes](../bir/codegen-helpers-kernel-provision.md) (Part 7, planned).*
...
- [BIR Kernel-Inst Nodes](../bir/codegen-helpers-kernel-provision.md) — the Part-7 node serializers (I28): the `InstBIRKernel`/`InstNKIKernel`/`InstNKIKLIRKernel` field layouts and the `InstructionType` enum
