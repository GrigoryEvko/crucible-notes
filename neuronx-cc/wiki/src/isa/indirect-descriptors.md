...
|---|---|---|---|---|---|
| INDEX ADDR4 | `+0x00` | `+0x00` | u32 | `getTensorIndirectIndicesAP()` (vtbl `+0x68`), `a4=0` | CONFIRMED |
| DATA ADDR4 | `+0x04` | `+0x04` | u32 | the gathered data AP, `a4=0` | CONFIRMED |
| SCALE ADDR4 | — | `+0x08` | u32 | the E8M0 scale AP, `a4=1` | CONFIRMED |
| NUM / x4-NUM | `+0x08` (u16) | `+0x0C` (u16) | u16 | `getNumIndirectIndices()` (vtbl `+0x90`); ×4 on MX iff Dtype∈{2,8,9} | CONFIRMED |
| SCALE base-part | — | `+0x0F` (u8) | u8 | `scalePart % PE_count` | CONFIRMED |
| indirect marker | `+0x03`.bit5 | `+0x03`.bit5 | bit | `or [+3],0x20` on the INDEX ADDR4 | CONFIRMED |
...
`assignAccess3D` handles **both** the static-3D and the indirect-3D case off one fork. The mangled symbol is `assignAccess3D`, but the body is the template `assignIndirectPattern<NEURON_ISA_TPB_INDIRECT16B>` (CONFIRMED — rodata context-name `0x1d71fa8` `"assignIndirectPattern<…INDIRECT16B>"` loaded at entry):
...
> **CORRECTION (D-N05 C-1) — the indirect branch does NOT fall through to the `TENSOR3D` static fill.** A prior preliminary reading held that `INDIRECT16B` "uses the `TENSOR3D` static pattern." The full disassembly shows the *same* `assignAccess3D` **forks** on `isTensorIndirectDynamicAP()`: the indirect branch writes only `{INDEX@+0, DATA@+4, NUM u16@+8, +bit29@+3}` and **returns** at `0x150ceea`; only the *non-indirect* branch tail-calls `assignStaticPattern<TENSOR3D>` (`0x150c390`). So on the wire an `INDIRECT16B`'s bytes `+0xA..+0xF` are inert — not stride/num-filled. The "16B" suffix names the static sibling's slot width (which is also the slot the indirect record sits in), not its byte content. Fixed in place. CONFIRMED from disasm of both branches.

The count comes from `getNumIndirectIndices()` (vtbl `+0x90`), **not** the partition-element count, and is **not** ×4-scaled — there is no `shl eax,2` between the `call [+0x90]` (`0x150cdef`) and the `mov [rcx+8],ax` (`0x150ce0b`). See the §"Decoder validation" CORRECTION for why this slot is `+0x90` and not `getNumElementsPerPartition`.
...
|---|---|---|---|
| MX vs non-MX | MX → `MXINDIRECT16B`; non-MX → `INDIRECT16B/20B` | `assignAccessForMX` self-routes on vtbl `+0x80` | CONFIRMED |
| 16B vs 20B (non-MX) | 3-free-dim → 16B; 4-free-dim → 20B | AP free-dim count picks `assignAccess3D`/`4D` | CONFIRMED |
| indirect vs static | every `assignAccess*` | `isTensorIndirectDynamicAP()` (vtbl `+0x80`) | CONFIRMED |

| Descriptor | BIR op(s) | Kernel / codegen leaf | Confidence |
|---|---|---|---|
| `MXINDIRECT16B` | `InstMatmultMx` / `InstQuantizeMx`, when an MX operand AP is tensor-indirect → `assignAccessForMX` → `MXINDIRECT16B` | MoE expert/token DGE gather: `backend::ExpertMLPsTKGKernel` (`lowerKernel` `0xd88640`) — `computeGateUpProj`/`DownProj` `For{Tokens,Experts}` | STRONG |
| `INDIRECT16B` (2-ADDR4, 3-D) | `InstGather` (92) ← `klr::NcNGather`; `InstIndirectCopy` (26) ← `klr::NcLocalGather`; `InstIndirectLoad/Save` (43/45); `InstGenericIndirectLoad/Save` (42/44) | per-partition `int32` row gather (`NcNGather`); 16-partition-block `u16`-offset SBUF gather (`NcLocalGather` → `IndirectCopy`) | STRONG |
| `INDIRECT20B` (2-ADDR4, 4-D) | same op family with a 4-free-dim index/data AP | same kernels, 4-D index/data geometry | STRONG |
| *(scatter)* | `InstIndirectSaveAccumulate` (46) — opcode `0xCA`, plain-ADDR4 RMW (no `0x20` mode bit; CoreV2) | MoE scatter-add / embedding-bag back-write — the scatter mirror of the gathers | STRONG |
...
> **NOTE — `GenericIndirectLoad/Save` are concretized *before* reaching these encoders.** `GenericIndirectLoad(42)`/`Save(44)` are expanded by a `LowerGenericIndirect` pass (`codegenIndirectLoadSave` `0xb580e0`) into concrete offset arithmetic + an indirect DMA. By contrast `Gather(92)`/`IndirectCopy(26)` are **already** concrete (a single resolved index axis) and flow straight to their `CoreV*GenImpl::visitInst*` emitters, which call `assignAccess3D/4D` to materialize the `INDIRECT16B/20B` record. The DMA-engine indirect descriptor (opcode `0xBB`) takes a different validator (see below). The op-number bindings are STRONG — assembled from the codegen-leaf symbols and the MX-operand routing — not byte-fixture CONFIRMED; no emitted `.neff` gather descriptor was diffed.
...
> **CORRECTION (D-N05 C-2) — the `+0x90` vtable slot is `getNumIndirectIndices`, not `getNumElementsPerPartition`.** A prior pass labeled `AP.vtable+144` (= `+0x90`) as the per-partition element count. The `libBIR.so` disassembly settles it: the base `getNumIndirectIndices() @0x20fed0` is `31 c0 c3` — `xor eax,eax; ret` (a virtual, overridden in `PhysicalAccessPattern`/`SymbolicAccessPattern`), whereas `getNumElementsPerPartition() const @0x202060` is a **non-virtual** `add rdi,0x50; jmp …` accessor of `Pattern+0x50` — a different function, not the `+0x90` slot. The indirect branch is reached only when `isTensorIndirectDynamicAP() == true`, so the count is the **gather index count**. Fixed in place. CONFIRMED from `libBIR` disasm of both accessors.
