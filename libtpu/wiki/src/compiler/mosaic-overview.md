...
> **CORRECTION — the driver entry is `CustomCallEmitter::Emit`, not `MosaicEmitter`.** The [compiler overview](overview.md) names "`MosaicEmitter`" as the side-door emitter. In this binary `MosaicEmitter` (`EmitWindow` @ `0xfaadcc0`) is a small anonymous-namespace **window-emit helper** (a sibling of `MosaicBroadcast` and `PipelineEmitter::OperandWindow`) used *inside* the windowing step; the class that actually imports the module, validates the ABI, runs the pipeline, and stitches LLO is `CustomCallEmitter::Emit` (`0x111ef740`). Treat "MosaicEmitter" in the overview as a loose name for the Mosaic side door; the driver symbol is `CustomCallEmitter::Emit`.
...
| `MosaicEmitter` is a window-emit helper, not the driver (overview-name correction) | `MosaicEmitter::EmitWindow` `0xfaadcc0`, sibling of `MosaicBroadcast`/`PipelineEmitter::OperandWindow`; not called as the lowering driver | HIGH |
