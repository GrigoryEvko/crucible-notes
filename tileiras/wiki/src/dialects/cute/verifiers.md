...
[TypeID Sentinels and Anchors](../../mlir-infra/typeid-sentinels-and-anchors.md) documents the upstream-MLIR sentinel idiom that the cute kind discriminator mirrors at the dialect level. [Layout Algebra and Descriptor Grammar](layout-algebra-and-descriptor-grammar.md) covers the layout primitives whose Types carry these sentinels. [Atom Builders and Desugar](atom-builders-and-desugar.md) covers the desugar pass whose rewrites the verifiers run against. [Tile and Divide Ops](tile-and-divide-ops.md) documents the higher-level tile partitioning ops whose verifiers reuse the rank, mode, and coordinate gates listed here.
