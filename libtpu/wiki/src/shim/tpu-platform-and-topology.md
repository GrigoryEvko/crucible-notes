...
> **CORRECTION (PLAT-1) —** the task brief listed `TpuPlatform_GetTpuHostLocation`, `TpuPlatform_TpuMemoryLimit`, and `TpuNodeContext_StopChipHeartbeats` as roster members. Symbol-table evidence overturns this: in this build there is **no** `TpuPlatform_GetTpuHostLocation` or `TpuPlatform_TpuMemoryLimit` C-ABI free function and **no** `StopChipHeartbeats` symbol at all. `GetTpuHostLocation` and `TpuMemoryLimit` exist only as C++ *methods* on the SE `tensorflow::tpu::TpuPlatform` class (`…TpuPlatform::GetTpuHostLocation() const @ 0xe999f60`, `…TpuPlatform::TpuMemoryLimit(long*) @ 0xe99a2c0`), and `TpuMemoryLimit` is exposed over the C ABI only under a *different* roster, `TpuConfigurationApi_TpuMemoryLimit @ 0xe8cdc40`. The C-ABI `TpuPlatform_*` roster's nearest equivalents are `GetHostLocation` (host-location handle, §2) and — for memory — nothing; memory limit is a configuration-API concern. This page documents the verified 11 + 5; the three brief names are recorded here as not-in-roster.
