...
|----:|------|------:|--------:|:----------:|-------|:----------:|
| 0 | `uint8` | 1 | `0x03` (3) | 1 | unsigned int 8 | CERTAIN |
| 1 | `int8` | 1 | `0x02` (2) | 1 | signed int 8 | CERTAIN |
| 2 | `float4_e2m1fn_x4` | 2 | `0x10` (16) | **4** | FP4 E2M1 ×4 | CERTAIN |
| 3 | `float8e3` | 1 | `0x0d` (13) | 1 | FP8 (legacy e3) | CERTAIN |
| 4 | `float8e4` | 1 | `0x0e` (14) | 1 | FP8 (legacy e4 ≡ e4m3) | CERTAIN |
| 5 | `float8_e4m3fn` | 1 | `0x0e` (14) | 1 | FP8 E4M3 (finite) | CERTAIN |
| 6 | `float8_e8m0fnu` | 1 | `0x03` (3) | 1 | **E8M0 block-scale** | CERTAIN |
| 7 | `float8e5` | 1 | `0x0f` (15) | 1 | FP8 (legacy e5 ≡ e5m2) | CERTAIN |
| 8 | `float8_e4m3fn_x4` | 4 | `0x0e` (14) | **4** | FP8 E4M3 ×4 | CERTAIN |
| 9 | `float8_e5m2_x4` | 4 | `0x0f` (15) | **4** | FP8 E5M2 ×4 | CERTAIN |
| 10 | `uint16` | 2 | `0x05` (5) | 1 | unsigned int 16 | CERTAIN |
| 11 | `int16` | 2 | `0x04` (4) | 1 | signed int 16 | CERTAIN |
| 12 | `bfloat16` | 2 | `0x06` (6) | 1 | BF16 (1-8-7) | CERTAIN |
| 13 | `float16` | 2 | `0x07` (7) | 1 | FP16 (1-5-10) | CERTAIN |
| 14 | `uint32` | 4 | `0x09` (9) | 1 | unsigned int 32 | CERTAIN |
| 15 | `int32` | 4 | `0x08` (8) | 1 | signed int 32 | CERTAIN |
| 16 | `float32` | 4 | `0x0a` (10) | 1 | FP32 (1-8-23) | CERTAIN |
| 17 | `float32r` | 4 | `0x0b` (11) | 1 | **FP32 reduced (TF32-like)** | CERTAIN |
| 18 | `uint64` | 8 | `0x01` (1) | 1 | unsigned int 64 | CERTAIN |
| 19 | `int64` | 8 | `0x0c` (12) | 1 | signed int 64 | CERTAIN |
...
The first 20 bytes of `byte_1DFBAD0` are the table; the four trailing bytes (`11 15 17 13`) are a *different* adjacent LUT, kept out of range by every consumer's `cmp …,0x13` bound. The stride array runs exactly 20 qwords before the `.rodata` strings begin — there is no 21st entry. Both arrays are byte-identical to the master table on [BIR Dtype Tables](../bir/dtype-tables.md) (7.6) and to the `_DTYPE` enum on [ISA enum-ordinals](../isa/isa-enum-ordinals.md) (2.23).
...
The **container** byte-size is what drives address arithmetic: `qword_1DFC040[2] = 2` and `[8] = [9] = 4` — *not* the logical lane width (which would be 0.5 / 1 / 1 byte). The matmul/ldweight encoder folds this stride into the start address as `start = base + (numContainers − 1) · stride[dtype]`, walking the data region by packed containers.

*Anchors: the encoder fold is `imul rbx, qword[0x1dfc040+rax*8]` in `generateLdweightMx` @0x143e350.*
...
The `(unsigned)(dt - 8) <= 1` idiom tests `dt ∈ {8, 9}` in a single unsigned compare; together with the explicit `dt == 2` it covers exactly the x4 set `{2, 8, 9}`. In the CoreV4 MX descriptor encoder (`assignAccessForMX<MXMEM_PATTERN1D>` @0x150e2f0) this is the disasm sequence `cmp edx, 2` (0x150e603) / `cmp edx, 1` (0x150e60b, on `dt − 8`) / `shl eax, 2` (0x150e610, the `× 4`), with the result stored as the K-extent `WORD` at descriptor offset `+0x8`. The same rule appears in the BIR simulator's `visitInstMatmultMx` (`Ki *= 4 iff dtype∈{2,8,9}`) and in the MX verifier — one rule, three consumers, byte-exact.
...
The host compiler reasons about dtypes through a small set of predicates. The `_x4` machinery is re-exported into `support/dtype.so` from the external `neuron_dtypes` C extension (the façade module performs a single `from neuron_dtypes import …` and re-exports 45 names; it defines **no** function bodies of its own). The names below are all present in the façade's string pool, but their bodies live in `neuron_dtypes`, which this wheel does not ship — so the semantics are reconstructed from name plus call shape and cross-checked against the in-binary encoder/verifier behaviour.

| Predicate | Signature | Returns | Confidence |
|---|---|---|:--:|
| `is_x4_dtype(dt)` | `dtype → bool` | `dt ∈ {float4_e2m1fn_x4, float8_e4m3fn_x4, float8_e5m2_x4}` | HIGH |
| `get_x1_from_x4(dt)` / `x4_to_x1(dt)` | `dtype → dtype` | the scalar element dtype of the container (e.g. `…_e4m3fn_x4 → float8_e4m3fn`) | HIGH |
| `launder_x4_dtype(dt)` | `dtype → dtype` | strips the `_x4` tag → the underlying x1 element dtype | HIGH |
| `sizeinbytes(dt)` | `dtype → int` | container byte-size (matches `qword_1DFC040[Int]`) | HIGH |
| `is_float_type(dt)` / `is_int_type(dt)` | `dtype → bool` | format-family membership | HIGH |
| `dtype2str(dt)` / `str2dtype(s)` | name ↔ dtype round-trip | (matches `bir::Dtype2string` / `string2Dtype`) | HIGH |

### `is_x4_dtype` and `launder_x4_dtype`

```c
// support/dtype.so → neuron_dtypes (re-exported; body external)
// names present in the __pyx_k_ pool: is_x4_dtype, launder_x4_dtype,
//   get_x1_from_x4, x4_to_x1  — semantics INFERRED (set-membership + element-map)
...
The naming `launder` is literal: an `_x4` container dtype is "laundered" through the scalar type-system so that promotion and merge logic — which only understands x1 element types — can reason about it, after which the `_x4` tag is restored on the result container. The `promote_type` and `merge_type` callers launder x4 inputs to their element dtype *before* promotion, then re-tag the container. `support/dtype.so` carries all four names (`is_x4_dtype`, `launder_x4_dtype`, `get_x1_from_x4`, `x4_to_x1`) plus the three `_x4` singletons; the set-membership and element-map semantics above are [INFERRED] from those names together with the encoder's `{2,8,9}` gate.

> **NOTE —** the binary carries the four x4 element-dtype names as a slightly inconsistent pool. The forward FP4 cast is spelled `static_cast_fp32_to_float4_e2m1fn_x4`, but the reverse is `static_cast_float4_e4m2fn_x4_to_fp32` — `e4m2` instead of `e2m1`. Both spellings are present in the binary; the reverse name is an upstream typo. Both refer to the same 4-bit E2M1 format. A name-based loader must accept the typo'd spelling.
...
|---|---|---|:--:|
| `static_cast_fp32_to_bfloat16` / `…_bfloat16_to_fp32` | BF16 = top 16 bits of FP32 | RNE (ties-to-even) | HIGH |
| `static_cast_fp32_to_float8_e4m3` / `…_e4m3_to_fp32` | FP8 E4M3 (1-4-3) | RNE, saturate-to-max | HIGH |
| `static_cast_fp32_to_float8_e5m2_x4` / `…_e5m2_to_fp32` | FP8 E5M2 (1-5-2, has Inf/NaN) | RNE | HIGH |
| `static_cast_fp32_to_float4_e2m1fn_x4` / `…_e4m2fn_x4_to_fp32`(*) | FP4 E2M1 (1-2-1) | RNE | HIGH |
| `static_cast_fp32_to_float8_e8m0fnu` / `…_e8m0fnu_to_fp32` | E8M0 (8-bit exponent, no mantissa) | — | HIGH |
| `static_cast_fp32_to_fp32r` / `…_fp32r_to_fp32` | FP32r reduced (TF32-like) | mantissa re-round | HIGH |
...
> **GOTCHA — the device E8M0 "cast" is a raw byte, not a power-of-two.** The OCP `2^(e − 127)` decode above is the *host* reference only. The device twin `bir::CastToNewDType` decodes E8M0 (source tag 6) as a raw `uint8 → float` — a `movzx` with **no** scaling — materialising the stored exponent byte as an integer. Host and device are therefore not bit-identical for E8M0. The power-of-two application happens later, in the MX dequant: the simulator's `dequantizeMx` @0x27b040 computes `out = ldexpf(data, scale[block] − 127)`, applying the shared block exponent per 32-element (8-partition × 4-column) OCP block. Treat an E8M0 "cast" as exponent **bookkeeping**, not a value conversion.
...
All narrowing FP encoders round **RNE only** (round-to-nearest-even; explicit guard+sticky, `fesetround(FE_TONEAREST)`); float→int paths use `roundss imm=0xC` (nearest, suppress-exception) + `cvttss2si` + a saturating `cmov`. There is **no** stochastic rounding anywhere in the cast surface. The saturate constants (E4M3 `0x7E`, E5M2 `0x7B`) and the RNE-only policy come from the device `CastToNewDType` constants; the FP4 value set is from the E2M1 spec.

### FP32r — reduced-precision FP32

`float32r` (Int 17) is a first-class 4-byte dtype, **not** a flag on `float32`. It carries its own wire-tag `0x0b` (11) and is sized/aligned identically to `float32` (stride 4, align 4), but it holds a TF32-style truncated-mantissa fp32. The cast pair re-rounds the mantissa to the fp32r grid and widens back. It ranks **above** plain `float32` in the promotion precedence ladder (next section). The simulator's MAC special-cases it: `matmultImpl` calls `sub_2A4750` to round/truncate both operands when `ap.getType() == 17`. (The host cast body lives in the external `neuron_dtypes`.)
...
```c
// promote_type(t0, t1)  @ penguin/dtypes.so
...
The four nested-closure qualnames (`promote_type._is_floating` / `._promote_floats` / `._promote_ints` / `._promote_float_int`), the `__pyx_scope_struct__promote_type` cell struct, and the error strings `"No available implicit dtype promotion path…"` and `"Cannot merge type!"` are all present in `penguin/dtypes.so`, and the dispatch plus the float-precedence ladder are read off the disassembly. Only the mixed-sign integer result *widths* are [INFERRED] from the ladder targets. The full lattice is the subject of [Numerics §9.2](dtype-promotion-lattice.md).
