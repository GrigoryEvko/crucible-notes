...
| [hal-registers](hal-registers.md) | register and reg-offset accessors | the `aws_reg_*`/`aws_hal_get_*` accessor *mechanism* (trampolines + inlined writers); the per-arch register/offset *return values* belong to [arch-csr-offsets](arch-csr-offsets.md) |
