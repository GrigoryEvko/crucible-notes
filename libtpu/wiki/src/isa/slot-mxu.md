...
> **CORRECTION (MXU-JF-DWG) —** an earlier reading labeled the `cmp $2` selector in `EmitVectorMatmul` the `MatmulDataFormat` (treating "value 2" as a data-format ordinal). Decompilation shows the compared argument is the **`DoneWithGainsMode`** (param `a5`), and value 2 is `DoneWithGainsMode::kTransposed`. The same function holds the assertion `dwg_mode != DoneWithGainsMode::kTransposed` guarded by *"JF/DF have only one GSF"* (`jellyfish_emitter.cc:1710`), confirming `dwg` is the transpose/gain-finalize mode. `MatmulDataFormat` is a *separate* parameter; on Jellyfish it does not steer this opcode dispatch.
