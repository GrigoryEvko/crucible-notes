...
- [VPU Slot](slot-vpu.md#predicate-and-vector-mask-register-files) — the `Vmsk` vector-mask register file from the consumer side (the 16-deep select-consumable view of the M-register file) and the `VectorSelectVmsk` ops.
- [VEX Mask / Dest-Port / Sub-Opcode](../sparsecore/vex-mask-destport-subopcode.md) — the owner of the SC VEX `bit0x104..0x108` 5-bit M-selector field that *names* the M-register this page builds; its `GetVectorMask` decode (`0x13a33320`) is the same one cited here.
...
- [Scan Datapath](../sparsecore/scan-datapath.md) — the consumer side of the M-register: how the scan-emit arm attaches the M-selector (`proto+0x38`) and the in-scan inactive-lane-identity model.
