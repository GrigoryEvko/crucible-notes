...
| instruction class | 22 encoder case bodies (1 zero-base default + 21 BarnaCore classes) | the `switch (opcode)` in `getBinaryCodeForInstr` lowers to a **self-relative jump table at `0xaed7dac`** indexed by raw opcode (`movslq (%rcx,%rbx,4),%rdx; add %rcx,%rdx; jmp *%rdx` @ `0x13c74e94`, table base in `%rcx`): one default arm (target `0x13c74e9d`, zero base, copy-and-return) plus 21 populated arms over the `2855..3991` opcode band. Opcodes 0-498 are gated to the trap by `cmp 0x1f2 / jbe` before the table read, so the leading 499 entries are dead padding (all hold the offset `0x08d9d0f1`, i.e. the default arm). | CONFIRMED |
