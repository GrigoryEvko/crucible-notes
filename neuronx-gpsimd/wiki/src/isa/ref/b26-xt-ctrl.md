...
oracle. Confidence tags: `[HIGH/OBSERVED]` = read-from-byte / proven by the device
oracle; `[MED/INFERRED]` = reasoned over OBSERVED; `[…/CARRIED]` = re-used at a sibling page's
confidence.

> **NOTE — address arithmetic.** `libisa-core.so` (9 690 712 B, ET_DYN
> x86-64, not stripped). Per `readelf -SW`: `.text` (VMA `0x312c10` ↔ file `0x312c10`) and
...
`nm | rg -c "Opcode_<mnem>(_w15)?_Slot_"` presence sweep (**0 missing of 60**). The
...
Most groups are **semantic singletons** (group name == mnemonic). The only two
...
This was VERIFIED by distinct-register round-trips (e.g. `mull a5,a6,a7`
...
> where the 8-bit signed displacement byte happened to dress up `byte0`. Re-grounded with a
...
AR-file scalar load from local data memory (or, for `L32R`, the PC-relative
...
**Round-trip** (`--no-transform`, so the assembler keeps the wide form):
...
literal-pool form has no FLIX copy).
...
has identical literal-load latency (`art@5`).

---

## 2. Literal-immediate build — `CONST16`

Two-instruction 32-bit constant build *without a literal-pool round-trip*.
...
Round-trip: `const16 a3, 0x1234 → 123434` (the `imm16` fills the narrow field,
...
AR-file scalar store to local data memory. `vAddr = ars + (imm << scale)`; write
...
Round-trip: `s32i a3,a4,252 → 3f6432`, `s16i a3,a4,4 → 025432`,
...
2-cycle producer can feed a store with **no stall** (data consumed late, `@5`).

---

## 4. Conditional branch — 24 ops

Compare two AR values (reg-reg), an AR vs a small immediate, or an AR vs zero, and
...
standalone fetch path uses the `_inst` form documented here.
...
`[23:16]`. The **condition selector is `r=[15:12]`**, verified by a 12-way round-trip:
...
The pairs `BNONE/BANY`, `BEQ/BNE`, `BLT/BGE`, `BLTU/BGEU`, `BALL/BNALL`, `BBC/BBS`
...
Unconditional control transfer. Two encoding families share this batch:
...
Each thunk is exactly `c7 07 <imm32>  c3` (`movl $imm,(%rdi); ret`). Round-trip,
...
The Xtensa code-density option: 16-bit-wide re-encodings of the most common ops,
...
Round-trip (auto-narrowed, no `--no-transform`):
...
Two density quirks, both verified by raw-byte round-trip:
...
The 32×32 integer multiplier. `MULL` = low 32 bits of the product; `MULUH`/`MULSH`
...
`op0` sub-major). Field map VERIFIED by isolating every field with `mull a5,a6,a7`:
...
`@6`; a dependent op stalls ~2 cycles). **Fully pipelined**, 1/cycle throughput.

---

## 8. `xt_sem_div` — scalar integer divide unit, 4 ops

The scalar core integer divider (distinct from the IVP vector `ivp_sem_divide` of
...
Note the N-slot asymmetry: `QUOS` places on `N0`+`N2`, `MULL` on `N1`+`N2` —
...
**60 opcodes**, count CHECKS exact: 9 core ld/st + 24 branch (12 `bst8` + 8
`bsi8` + 4 `bsz12`) + 8 jump/call + 10 density `.N` + 5 `xt_mul32` + 4 `xt_sem_div` = **60**. A
`nm libisa-core.so | rg -c "Opcode_<mnem>(_w15)?_Slot_"` presence sweep returned **0
missing of 60**.

All 60 mnemonics **round-tripped** `xtensa-elf-as → xtensa-elf-objdump`
...
**Selector CONSTs read out of the encode thunks' `movl $IMM,(%rdi)` bytes**:
...
**`ncore2gp` is an 8-only windowed config**: `call4`/`call12`/`callx4`/`callx12` are
...
**Timing** from `INSTR_SCHEDULE` USE/DEF stage deltas
...
**ABI**: these are SCALAR base-Xtensa AR-file ops — **no `CPENABLE`** state-in, **no
...
> **CORRECTION carried forward.** The reg-reg branch major-op is **`op0=0x7`** (re-grounded with a
> zero-displacement round-trip), not the displacement-contaminated `0x2` an earlier
