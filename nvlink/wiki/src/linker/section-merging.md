...
| `SHT_CUDA_CALLGRAPH` (0x70000001) | Inline in `merge_elf` | Callgraph edge table: 8-byte records whose meaning is governed by a sticky type marker carried in zero-caller sentinel records. Caller symbol indices are remapped through the symbol mapping table; dead (deleted) symbols are skipped. Four edge types dispatch to four builders: type `-1` (`sub_44B9F0`, direct callee_list at node+16), `-2` (`sub_44BA60`, interns name into string table and writes its string-table offset at node+4 (the "name token" for this address-taken symbol), stamps address_taken=1 at +50), `-3` (`sub_44BAA0`, alt_call_list at +8, name-keyed), `-4` (`sub_44BF90`, fn_ptr_edge_list at +32, stamps callee's address_taken). |
