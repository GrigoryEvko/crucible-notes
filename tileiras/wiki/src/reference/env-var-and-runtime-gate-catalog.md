...
T3 -- deep reference, complete env-var and gate enumeration.

## Confidence

HIGH for env-var names extracted from `.rodata` -- every name in Table 1
is a verbatim string literal cross-referenced to its `getenv` call site
(see `sub_2D381B0`, `sub_7A9D60`, `sub_7B6970`, `sub_8D9DD0`,
`sub_12C8DF0`, `sub_5773C0`, `sub_1A41D30`, and the LLVM-Support
helpers). HIGH for gate addresses, populating ctors, and `cl::opt`
flag-name bindings -- each row is backed by a decompiled ctor body that
shows both the literal default-value store and the `sub_4534CC0(..., name,
len)` registration call. MED for default values inferred from
getenv-with-default patterns where the binary's default-on-unset behavior
is encoded as a fall-through path rather than a literal RHS (notably
`TILEIR_PREFER_TMA_FOR_LOAD_STORE` defaulting to comparison-against-
`"false"`, and `TILEIR_DELAY_TMA_STORE_WAIT` returning `(*a1 == 4)` when
the env var is absent). MED on the precise runtime role of the
runtime-owned (no-ctor) gates inherited from earlier P2 reports.

## Sources

- Binary: `tileiras` (ELF64, 88 MB, stripped, CUDA Toolkit 13.1, build
  stamp `Fri_Nov__7_19:49:56_PST_2025`), SHA256
  `f0eb415767f403c96cbabf0817c3bcf70a50f88dfc8845fe36ebe21635fa6707`.
- `getenv` PLT stub: `0x004055B0`. `getenv`-into-`std::string` wrapper:
  `sub_45AE9A0` @ `0x045AE9A0`.
- Driver toolkit-root resolver: `sub_5773C0` @ `0x05773C0`. NVVM
  toolkit-path resolver: `sub_1A41D30` @ `0x01A41D30`.
- TILEIR/TILE\_AS consumers: `sub_2D381B0`, `sub_7A9D60`, `sub_7B6970`,
  `sub_8D9DD0`, `sub_12C8DF0`.
- Static ctors installing the `0x5B6xxxx` gate band: `ctor_370` --
  `ctor_387` and `ctor_611` (decompiled bodies under
  `decompiled/ctor_*.c`).
- LLVM CommandLine name-binding helper: `sub_4534CC0`. Option-storage
  layout follows the standard `[Initial]+0x10[Default]` two-field pattern
