...
[asm-printer-monster-and-windows.md](asm-printer-monster-and-windows.md) documents the dispatcher and mnemonic pool that finally print the template strings shown above. [tcgen05-wgmma-mbarrier-cluster.md](tcgen05-wgmma-mbarrier-cluster.md) covers the SM100/SM103 control word and the mbarrier/cluster wiring around `tcgen05.mma`. [iseldag-and-matchertable.md](iseldag-and-matchertable.md) shows where the selector chooses between inline-asm and `MachineInstr` paths for each SM tier. The MMA atom registry in [../dialects/cute_nvgpu/mma-atoms-sm70-120.md](../dialects/cute_nvgpu/mma-atoms-sm70-120.md) is the dialect-level entry point that feeds shape and operand types into these templates.
