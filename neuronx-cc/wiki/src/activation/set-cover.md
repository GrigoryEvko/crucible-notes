...
The LUT refill has **no cost model** in this build (the `bir::Hwm::getLatencyExec(InstLoadActFuncSet, …)` methods are `__assert_fail` stubs — [1.10](../arch/activation-engine.md), [10.6 §0](loadactfuncset.md)). The scheduler therefore cannot charge a typed cycle cost for a reload; it hides the (physically real) refill *structurally* by floating each `LoadActFuncSet` to its earliest legal program point, behind upstream compute.
