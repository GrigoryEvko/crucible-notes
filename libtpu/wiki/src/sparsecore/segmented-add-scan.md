...
- **The segment boundary is operand 1 of `SegmentedScanOp`, not a control register.** `build` adds two `Value` operands in order — `(data, segment-id)` — plus the `reduction_op` `StringAttr`. The segment id is allocated to a V read port (INFERRED V1 by build order). There is no SparseCore `SetSegmentPattern`.
