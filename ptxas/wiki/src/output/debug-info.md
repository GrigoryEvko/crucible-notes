...
ptxas generates DWARF-based debug information for cuda-gdb and other GPU debuggers. The debug subsystem spans three distinct code regions:

- **Early-pipeline DWARF line table generator** (`0x45A`--`0x45C`) — encodes PTX-level source mappings.
- **Mid-pipeline SASS-level emitter** (`0x860`--`0x868`) — produces both `.debug_line` and `.nv_debug_line_sass` sections along with register mapping tables.
- **Late-stage DWARF processor/dumper cluster** (`0x1CBF`--`0x1CC9`) — handles `.debug_info`, `.debug_abbrev`, `.debug_loc`, and `.debug_frame` parsing and emission.

The design follows a two-tier model:

- **PTX-level debug info** records source file/line to PTX instruction mappings.
- **SASS-level debug info** records the final PTX-to-SASS address correspondence after all optimizations.

NVIDIA extends standard DWARF with proprietary sections (`.nv_debug_line_sass`, `.nv_debug_info_reg_sass`, `.nv_debug_info_reg_type`, `.nv_debug_info_ptx`) and Mercury-namespace variants (`.nv.merc.debug_*`) for Capsule Mercury binaries.
