...
Five blobs exist — jellyfish (v2), dragonfish (v3), pufferfish (v4), viperfish (v5p/v5e), ghostlite (v6e). The newest generation, `6acc60406` (v7x / Trillium), ships **no** accuracy table: its `embed://` lookup fails and the impl-type selector falls through to a warning-driven fallback. The first three generations carry byte-identical content (same 181,284-byte blob modulo the version stamp); viperfish and ghostlite each tighten the fast-path bounds by one to four orders of magnitude, in lockstep with the per-gen transcendental cycle-cost improvements.
...
Top-level worst-case **relative error** (field 6) per `(op, variant)` — lower = more accurate. `j/d/p` = jellyfish/dragonfish/pufferfish (identical).
...
1. **`GetTargetAccuracy(HloOpcode, Target&, bool variant)`** @ `0x1d60dea0` — the loader. Reads `Target+0x398` = `tpu_version()`, builds the `embed://` URI via `TpuVersionToString`, calls `tsl::ReadBinaryProto` to parse the blob, then `GetAccuracyEntry`. Returns `StatusOr<AccuracyEntry>` (NOT_FOUND if no table/entry).
2. **`GetAccuracyEntry(string_view name, AccuracyTable&, bool variant)`** @ `0x1d60e040` — linear scan over `table.entries`, matching `(entry.name == name) && (entry.variant == variant)`.
3. **`CompareAccuracy(ResultAccuracy&, StatusOr<Entry> fast, StatusOr<Entry> accurate, HloOpcode)`** @ `0x1d60d7a0` — the tolerance check. Converts the requested `ulps` to a relative error via `ulps × 2^-23` (the `2^-23 = 1.1920928955078125e-07` constant @ `0xa2dfb58`), rejects the fast path if the request exceeds `worst_rel_err`/`worst_ulp`, then walks the field-8 breakpoints to find where the request is met. Diagnostics: `"Only accurate implementation is available."`, `"Only EUP implementation is available."`, `"No implementation found for requested accuracy:"`.
4. **`GetImplType(Target&, ResultAccuracy&, HloOpcode)`** @ `0x1d60db80` — the master decision. Loads both variants (`GetTargetAccuracy(...,0)` and `(...,1)`) and switches on `ResultAccuracy.mode`: **DEFAULT** returns the available impl; **HIGHEST** forces the EUP path; **TOLERANCE** calls `CompareAccuracy`. An unhandled mode FATALs `"Unsupported result accuracy for opcode "`.

```text
no-table fallback (taken for 6acc60406 / Trillium, which has no accuracy_table):
  no EUP table  → WARN "No accurate implementation is available, returning default implementation."
  no fast table → WARN "No EUP implementation is available, returning accurate implementation."
```

> **NOTE —** the `2^-23` ulps→rel-err multiplier is read by `mulsd` from `.rodata` @ `0xa2dfb58`; the Hex-Rays output does not surface the immediate, so the exact value rests on the disassembly anchor (HIGH). The decision *logic* — both `GetTargetAccuracy` calls, the DEFAULT/HIGHEST/TOLERANCE dispatch, the two fallback warnings, and the FATAL — is decompile-confirmed (CONFIRMED).
