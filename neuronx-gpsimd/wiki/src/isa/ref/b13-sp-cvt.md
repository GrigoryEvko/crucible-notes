...
`libisa-core.so` (`strings -a | rg sem_sp_cvt`) are where the page title comes from.
...
own arithmetic. The page default is `[HIGH/OBSERVED·exec]` for value facts and `[HIGH/OBSERVED]`
for encoding facts, per the [confidence model](../../reference/confidence-model.md); claims that
depart from that default carry an explicit tag.
...
> (`readelf -SW` — *not* libtpu's `0x400000`). Value side: `libfiss-base.so`
...
placement at `Slot_{f0,f1,f2,f3,f7}_s3_alu` + `Slot_n0_s3_alu`. The round-honoring
...
> name root is the non-ambiguous key.
...
slot** (the `movl` immediate in the `Opcode_ivp_<mnem>_Slot_f1_s3_alu_encode` thunk, byte-exact)
and its operand model read from the iclass `args`/`stateArgs` tables. `vec lane in/out` gives
...
not a bit-flip of the base form. Every row of §2.1–§2.3 is `[HIGH/OBSERVED]`; the non-`t` base forms
are additionally **proven by execution**, while the `t` predicated forms are OBSERVED at the
encoding level only (their value is the base leaf gated by a mask — §7 NOTE).
...
> byte-exact) — **no additive or single-bit relation** between slots (the `F0` form even
...
> `Opcode_<mnem>_Slot_<that-slot>_encode`.

### 2.1 int32 ↔ fp32 with a Q-format scale (`float`/`ufloat`/`trunc`/`utrunc`)

| mnemonic | fmt·slot | opcode-sel imm (F1_S3) | vec lane in→out | bytes | one-line semantics |
|---|---|---|---|---|---|
| `ivp_floatn_2x32` | F1·S3_ALU (+5) | `0x27803000` | int32 → fp32 (16×32b) | 16/8 | `fp32(int32) · 2^(−imm5)` (signed Q-format) |
| `ivp_floatn_2x32t` | F1·S3_ALU (+5) | `0x27000000` | int32 → fp32, predicated | 16/8 | `float`, guard-lane masked |
| `ivp_ufloatn_2x32` | F1·S3_ALU (+5) | `0x27803400` | uint32 → fp32 | 16/8 | `fp32(uint32) · 2^(−imm5)` (unsigned) |
| `ivp_ufloatn_2x32t` | F1·S3_ALU (+5) | `0x27000400` | uint32 → fp32, predicated | 16/8 | `ufloat`, guard-masked |
| `ivp_truncn_2xf32` | F1·S3_ALU (+5) | `0x25a18000` | fp32 → int32 | 16/8 | `int32(fp32 · 2^imm5)` toward-zero, **saturating** |
| `ivp_truncn_2xf32t` | F1·S3_ALU (+5) | `0x28080000` | fp32 → int32, predicated | 16/8 | `trunc`, guard-masked |
| `ivp_utruncn_2xf32` | F1·S3_ALU (+5) | `0x25a98000` | fp32 → uint32 | 16/8 | `uint32(fp32 · 2^imm5)`, saturating |
| `ivp_utruncn_2xf32t` | F1·S3_ALU (+5) | `0x28088000` | fp32 → uint32, predicated | 16/8 | `utrunc`, guard-masked |

### 2.2 fp32 round-to-integral (result stays fp32)

| mnemonic | fmt·slot | opcode-sel imm (F1_S3) | vec lane in→out | bytes | one-line semantics |
|---|---|---|---|---|---|
| `ivp_firoundn_2xf32` | F1·S3_ALU (+5) | `0x2e0000b0` | fp32 → fp32 | 16/8 | round to integral, **half-away-from-zero** (fixed) |
| `ivp_firoundn_2xf32t` | F1·S3_ALU (+5) | `0x27005030` | fp32 → fp32, predicated | 16/8 | `firound`, guard-masked |
| `ivp_ficeiln_2xf32` | F1·S3_ALU (+5) | `0x2e000020` | fp32 → fp32 | 16/8 | round to integral toward **+∞** (ceil) |
| `ivp_ficeiln_2xf32t` | F1·S3_ALU (+5) | `0x27005090` | fp32 → fp32, predicated | 16/8 | `ficeil`, guard-masked |
| `ivp_fifloorn_2xf32` | F1·S3_ALU (+5) | `0x2e000030` | fp32 → fp32 | 16/8 | round to integral toward **−∞** (floor) |
| `ivp_fifloorn_2xf32t` | F1·S3_ALU (+5) | `0x27005020` | fp32 → fp32, predicated | 16/8 | `fifloor`, guard-masked |
| `ivp_firintn_2xf32` | F1·S3_ALU (+5) | `0x2e0000a0` | fp32 → fp32 | 16/8 | round to integral, **FCR RoundMode** (RNE default) |
| `ivp_firintn_2xf32t` | F1·S3_ALU (+5) | `0x270050a0` | fp32 → fp32, predicated | 16/8 | `firint`, guard-masked |
| `ivp_fitruncn_2xf32` | F1·S3_ALU (+5) | `0x2e000021` | fp32 → fp32 | 16/8 | round to integral toward **0** (truncate) |
| `ivp_fitruncn_2xf32t` | F1·S3_ALU (+5) | `0x270050b0` | fp32 → fp32, predicated | 16/8 | `fitrunc`, guard-masked |

### 2.3 fp16 → fp32 widen (lands on the fp32 side)

| mnemonic | fmt·slot | opcode-sel imm (F1_S3) | vec lane in→out | bytes | one-line semantics |
|---|---|---|---|---|---|
| `ivp_cvtf32f16` | F1·S3_ALU (+5) | `0x27d050f0` | fp16 → fp32 | 16/8 | exact widen (mantissa 10→23, bias 15→127) |
| `ivp_cvtf32nxf16_0` | F1·S3_ALU (+5) | `0x27e050f0` | fp16[lo] → fp32 | 16/8 | widen low-half of 32-lane fp16 vector |
| `ivp_cvtf32nxf16_1` | F1·S3_ALU (+5) | `0x27f050f0` | fp16[hi] → fp32 | 16/8 | widen high-half of 32-lane fp16 vector |
| `ivp_cvtf32nxf16t_0` | F1·S3_ALU (+5) | `0x27005080` | fp16[lo] → fp32, predicated | 16/8 | widen low-half, guard-masked |
| `ivp_cvtf32nxf16t_1` | F1·S3_ALU (+5) | `0x27005010` | fp16[hi] → fp32, predicated | 16/8 | widen high-half, guard-masked |
...
> widen core, `@0x5b77f0`) — disassembled. `cvtf32f16` is the scalar/single-lane widen;
...
`Iclass_FLOAT_S_args` (`@0x854100`, file `0x654100`) lists **three** operands, read verbatim:
...
> reinterpret the signed truncation as `uint32` (so `−2 → 0xfffffffe`).
...
> with the directed modes available (`firint(2.5, RU)=3`, `RD=2`, `RZ=2`);
...
> correction) and [B20 hp-cvt](b20-hp-cvt.md) §5.2.
...
against the widen here, so it is stated for completeness (proven by execution):
...
The 23 mnemonics and their placement counts, counted with
...
**driven live**: `float`/`ufloat` (`@0x87aec0`/`0x87b0d0`), `trunc`/`utrunc`
...
> grounded — see §9.1: the convert writeback stage was read from the `libcas-core.so` ISS
...
5. **The fp32-convert family is 23 mnemonics / 138 placements in `S3_ALU`, disjoint from the
   ...
   in the **`S1_Ld`** slot (`nm` shows `Slot_f0_s1_ld_encode`), their value leaves are
   ...
   fp32 leg. `23 × 6 = 138` counted via `nm`. The two families share neither slot nor leaf.
   **Survives.**
...
the **final** `mov $0xN,%esi; jmp *%rax` is the destination (writeback) commit stage. Decoded from
the ISS (neither binary has DWARF — `readelf -S` shows only `.symtab`; both GCC 4.9.4-built — so this
is ISS disassembly, not debug info):
...
> stage (13), and `trunc`/`utrunc` one stage earlier (12). (Decoded from the `mov $0xN,%esi`
