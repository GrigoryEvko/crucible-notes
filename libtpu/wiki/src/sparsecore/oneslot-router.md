...
> **CORRECTION (ROUTER-1) —** the `VectorAlu` default-error string is "`Unsupported opcode for Vector Alu slot: $0 : $1`", not "while consuming Vector Alu slot." The scalar OneSlot router uses the "while consuming slot instruction" phrasing; the vector consumer uses "for Vector Alu slot." The two error strings are distinct `.rodata` literals, confirmed against the decompiled bodies; do not conflate them.
