...
The sibling barrier pages mark these integers **LOW** ("an embedded-memfile dependency / not statically extractable"). **That assessment is superseded here.** The chip-config blobs are *statically compiled into* `.lrodata`/`.rodata`; the so-called "memfiles" are not runtime-loaded — they are populated by load-time `R_X86_64_RELATIVE` relocations that point at compiled-in data. Every per-gen integer below was carved directly from the embedded blob bytes and cross-checked against the writer disassembly. The numbers are CONFIRMED, byte-exact.
...
> **CORRECTION —** [Barrier → SFLAG Binding](barrier-to-sflag-binding.md) and [SpecialPurposeSyncFlags](special-purpose-sync-flags.md) both mark these literals **LOW / "not statically extractable / memfile dependency."** Those pages are accurate about the *window geometry* (the carve, the `−5`, the five-slot map are gen-independent and CONFIRMED there) but conservative about the *integers*. The blobs are statically compiled in (§2); the integers are CONFIRMED byte-exact (§4). Treat this page as the authoritative source for the numbers; treat the sibling LOW markings as superseded.
