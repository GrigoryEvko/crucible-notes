...
> **CORRECTION (TEC-PAYLOAD-1) —** variant B's two trailing-width sub-branches (`13,16,16,22,1,1` vs `10,16,16,16,13,1,2`) both validate against the *same* CHECK (204) and the *same* oneof (55). Whether the inner branch is a second `packet_type` bit or a length-conditional re-decode was **not** isolated — the two sub-branches were observed to read slightly different trailing widths but converge on CHECK 204. Treat the two widths as a discriminated pair; the precise inner predicate is LOW confidence. The variant-A/B outer discriminator (`packet_type & 1`) and both CHECK constants are CERTAIN.
