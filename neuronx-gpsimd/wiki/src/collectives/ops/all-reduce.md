...
> field-range checker in the headers — there isn't one.
...
alignment; `sizeof == 64`, every offset compile-confirmed (`gcc offsetof`):
...
(`… pseudo_trigger_all_reduce;`). `[HIGH/OBSERVED — probe prints
...
> TOP SP instructions by the runtime."*
...
and the `first_ptc2_ins` symbol are both present.
...
says so verbatim — *"Same encoding as SDMA CCE op encoding"*.
...
`al_sdma_m2s_build_fma_descriptor @0x451c20`. `[both CARRIED/OBSERVED — struct
`size=140`, symbol present, called from 3 sites.]`
...
> byte-exact: `00 00 00 00 00 00 00 00`; adjacent `fp32`@idx10 = 4,
...
See [Collective Enum Reference](collective-enums.md).
...
Which numeric `enc_alg_type` a given (world-size, topology, dtype)
