...
- The **online-softmax algorithm in BIR**: the negated running max, the `min`-of-negated-maxes running-max update, the `α = exp(m_old − m_new)` rescale of the accumulator, and the causal `affine_select` iota-triangle mask — all read from `nkilib/core/attention/attention_cte.py`.
...
> **QUIRK — Cython obscures the exact `sema.<dispatch>` per intrinsic.** `TraceContext.call`'s control flow and the `_exclude_modules` tuple come straight out of the decompiled control flow and the interned strings, so the trace-versus-concrete *decision* is fully readable here. The precise `self.sema.<method>` reached for a given `nl.*` op is not: it routes through the module-state struct, and the per-op target only becomes visible one layer down, in Stage 2's `NeuronCodegen` method → `penguin.ir.<Op>` mapping.
...
So the "`IRBuilder.add_named_instruction → penguin.ir.<Op>`" the descent needs is the ctor-then-insert pair: `add_named_instruction` is reached *through* `self.insert`. The flash-attention body maps onto a small set of these methods, each building the named `penguin.ir.<Op>`:
...
When the trace finishes, the just-built `penguin.ir` kernel is stitched into the host graph by `insert_nki_kernel` (`TraceKernel.cpython-310.so` @ `0x42c20`). It builds a `BIRKernel` graph node (the producer form of the IT54 carrier; class from `penguin.ir.ir`), binds each shared-`hbm` input/output via `add_parameter` (operand types `TensorRef`/`MemrefTileND`/`ProgramId`/`DynamicScalar`), seals the body via `addBasicBlock`/`finalize_exec`, and validates every operand is in `shared_hbm` address space (guard string: "The input and output tensor of a NKI kernel must be in shared_hbm address space"). It selects **`ExternalNativeNkiKernel`** for a fresh user trace vs **`InternalMaterializedNkiKernel`** for a registry/`_private_kernels` leaf with cached weights — the producer-side twins of the Stage-3 consumers.
...
The IT55 NKI `bir::Function` body is the online-softmax algorithm, read from `nkilib/core/attention/attention_cte.py` (2920 lines, which ship as readable NKI Python). The mathematical target is `O = softmax(scale · Q·Kᵀ) · V` for prefill, with the KV axis processed in **sections** (8K-token flash-attn outer blocks, threshold `10·1024`) and running per-row statistics carried across sections:
...
Three subtleties a reimplementer must reproduce, each pinned to a source line:
