...
| 15 | MXU-result port | `GetResourceUsage` `Resource 3 → col15` | matmul | CERTAIN |
...
> **CORRECTION (cost-VF-OVR) —** an earlier synthesis named cols 2..9 the *vlxmr load-feed stages 1..4* (No/Xpose) and described `AddOverrunCheckReservations` as inserting a single value 5. The decompiled `AddOverrunCheckReservations` `@0x1c8abfe0` overturns both: it inserts the named enumerators `kMsrAOverrunCheck{0,1,2,3}` (cols 2,3,4,5) and `kMsrBOverrunCheck{0,1,2,3}` (cols 6,7,8,9) with a **graduated ramp `{5,13,21,29}`** (not a flat 5), each behind a distinct `try_emplace(...).second` CHECK string at `vf.cc:78..93`. The MSR-A set is taken on the `a1 != 1` (MSR0) path and the MSR-B set on `a1 == 1` (MSR1); `a1 ∉ {0,1}` is a `LogFatal("Invalid MSR.")` at `vf.cc:95`. The vlxmr feed still occupies cols 1 and 6..9/2..5 in its own rows, but the *overrun* augmentation is what gives cols 2..9 their authoritative names.
...
> **QUIRK —** the per-dtype throughput delta the [overview](mxu-latency-overview.md) describes as `{2,1,1}/{4,3,2}/{8,7,6}` does **not** live in the VF matpush *reservation* row. On Viperfish every key but f32-NoXpose pays the flat `{4,3,2}`; the dtype only forks on f32-vs-rest × transpose. The `{8,7,6}` int8-x8 magnitude lives in the separate `ViperfishPerformance` grid (res r3 = `{8,16,32}`, see [`performance-vf`](performance-vf.md)), not in this reservation array. A reimplementation that puts the per-format scaling in the matpush reservation will over-cost integer latches on VF.
