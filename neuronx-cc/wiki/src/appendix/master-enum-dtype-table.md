...
> *All ordinals, wire tags, and addresses on this page apply to `neuronx_cc 2.24.5133.0+58f8de22` (cp310). Enum ordinals and the L1↔L3 arithmetic are ABI-stable across cp310/cp311/cp312; the byte addresses are cp310-pinned (cp311/cp312 VAs drift). Backing reports: **D-D04** (dtype tables), **D-D14** (enum crosswalk), **D-V06** (ISA enum ordinals).*
...
The trap this table exists to prevent (D-D14 §0, the S2-04 §7-C5 warning): for several
...
The tables are grounded by re-deriving each enum from the binary — `libBIR`
`<Enum>2string` switch bodies (L1 names + ordinals), `libwalrus` wire LUTs / convert
encoders / name-decode switches (L3 bytes) — not by hand-copying the prose pages. The
five most error-prone enums (`Dtype` wire-tags, `AluOpType`, `Collective`,
`BranchCompareOp`, wait-mode) were **xxd-re-verified against `libwalrus.so` this pass**;
see [Adversarial Re-Verification](#adversarial-re-verification). Both `libBIR.so` and
...
```text
L1     = libBIR ordinal (the in-memory enum int; D-D01..D-D13, byte-exact off 2string)
...
Conf   = CERTAIN (byte off a switch/LUT this pass) | STRONG (byte certain, name joined 1:1
         from BIR 2string) | INFERRED (carried from a cross-report, flagged)
```
...
Dtype ordinal equals its wire tag (D-D04 §1, D-D14 §1.3, D-V06 ENUM 3). The enum is
exactly 20 members; the `2string` default is a `NeuronAssertion` at `Dtype.cpp:39` — no
case ≥20 exists. (This corrects S2-04 §3.6 which annotated the roster `(+more)`.)
...
- **x4** = lane-container expand factor 4 for the MX packed types `{2,8,9}`; the on-wire container holds 4 logical FP4/FP8 elements and `num_elem_per_partition` is ×4 on the MX matmul path (D-D04 §0).
...
> **CORRECTION (vs §7.6 / §9.1 wording) —** prose pages sometimes phrase FP4 alignment as a
> flat "align 1". Byte evidence: `core_v3::addr_aligned_dtype @0x136ed10` returns
> false/never-aligned for wire-tag 16 — FP4-x4 has **no defined core_v3 alignment**; it is
> "always-aligned (1)" only on the **core_v4** copy `@0x1446420`. The Align column above
> marks idx 2 "CoreV4-only" rather than "1" for this reason.
...
extended band pushes `mod_int` to `0xC8` (D-D14 §1.2, D-V06 ENUM 2). L2 keys:
...
> is INFERRED from P-3-213/NX-112, not exhaustively disassembled). `abs_max`/`abs_min`
...
subset `{bypass=0x00 (plain copy), add=0x04 (reduce-on-copy)}` is legal (D-V06 ENUM 6).
Likewise `_DGE_COMPUTE_OP` is an `AluOpType` reduced to `{none, add}`: `bypass`→`none`,
`add`→`add` via `backend::AluOpType2DGEComputeOp @0x120b9e0` (D-V06 ENUM 5).
...
the silicon byte is not the BIR integer (D-D14 §2):
...
255 = explicit `"Invalid"` sentinel. This is the L3 form of `InstructionType` (D-V06
ENUM 1) — the BIR `InstructionType` L1 ordinal differs from this opcode for almost every
op; the full 110-row L1→opcode join is a libwalrus Strand I/J deliverable (NX-125), so
only the MX trio + override roster are L1-pinned here.
...
> 0x10E3` — consistent with D-D14/D-B04. The MX class prefix `0x10` is the high byte; the
...
> (33, `Activate`) on CoreV2/V3 but `0x25` (37, `Activate2`) on CoreV4 (J27: `mov BYTE
...
instead. L2 key `cc_channel.kind` (D-D14 §1.6, D-V06 ENUM 12).
...
(`ord+3`, a +2 jump at the IMM→REG boundary). The semaphore enums (D-V06 ENUM 13/14):
...
noted (D-D14 §1.6, D-V06 ENUM 4/§M-ACCUM).
...
> **CORRECTION (RNG order, D-D10 C3 vs S2-04) —** an earlier note implied `PCG32=0`. The
> binary is authoritative: `LFSR=0, PCG32=1, PHILOX_1=2` (`2string @0x401de0`; `string2
> @0x40ef50` in order LFSR/PCG32/PHILOX_1).
...
a name (D-D14 §1.6). The activation func reaches silicon only indirectly: the chosen
func-id is baked into the `InstActivation`(IT4) opcode + the PWP table, and the **PWP
`neuron_id` is a SEPARATE id space** from this ordinal (D-D11).
...
(D-D14 §1.6 ⚑). No L3 silicon byte (it gates allocation/addressing).
...
> `byte[12] bits[0:2]`; how core_v5's runtime-raw path carries them is **unresolved** (GAP,
> D-D09 G2).
...
bitfields or packed windows (D-V06 ENUM 15/17/18/21):
...
| `_PE_FP32MODE` | bundle `+0x20`/`+0x21` | `{0: standard fp32, 2: fp32r 2-FMA round}`; fp32r signature `*(u16)(bundle+0x20)=0x020A` | STRONG |
| `_IMM_SRC` | validator only | 3 ordinals `{0:immediate, 1:register, 2:pointer}`; names not name-decoded (GAP) | CERTAIN (cardinality) / INFERRED (names) |

---

## Adversarial Re-Verification

The five highest-risk enums — the ones with layer reorders or wire-tag subtleties — were
**re-checked by `xxd` against `libwalrus.so` this pass** (not copied from prose). Every
LUT byte-matched the D-reports:
...
`AluOpType` comparison reorder cross-confirmed by the per-case `mov eax,IMM` sites cited in
D-D14/D-V06 (e.g. `case19 not_equal→0x18 @0x142e110`); `float32r` and `AllToAllV`/
`QuantizeMX` mnemonics confirmed present as standalone `.rodata` strings in `libwalrus.so`.
All five are tagged **CERTAIN**.
...
- **G3** — the full 110-row `InstructionType`→`_OPCODE` join: only the MX trio + override roster (`Activation`/`Exponential`/`Rand2`) are L1-pinned here; the rest of the per-op `setupHeader` words are a Strand I/J (NX-125 TABLE-A) deliverable.
- **G4** — the `AluOpType` int32 extended band (`0xC4..0xE1`) membership beyond `mod_int`(0xC8) is INFERRED from P-3-213/NX-112; the reordered comparison + abs/rsqrt + mod_int cases were re-disassembled.
...
- **G6** — no NEFF/BIR-JSON round-trip fixture byte-diff was performed; the L2 name↔L1 int binding is proven from (de)serializer bodies, the L3 byte from emitter constants — an end-to-end `.neff` hexdiff would upgrade the L2→L3 join from "emitter-constant CERTAIN" to "wire-observed".

---

## Cross-References

- [The BIR Dtype Enum and its Wire-Tag / Stride / Alignment LUTs](../bir/dtype-tables.md) — §7.6, full dtype LUT derivation (D-D04)
...
- [ISA Numeric Enum-Ordinal Tables](../isa/isa-enum-ordinals.md) — §2.23, the bit-pinned L3 silicon ordinals (D-V06)
