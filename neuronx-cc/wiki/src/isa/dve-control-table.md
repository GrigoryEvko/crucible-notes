...
| **Row stride** | **16 bytes**, every generation (`size / row_count`) |
| **Row count** | gen2 = 128; gen3/gen4 fast = 256, slow = 256 |
| **Effective word width** | gen2 = 96-bit (`bytes[12:16]==0`); fast = 64-bit (`bytes[8:16]==0`); slow = 112-bit (`bytes[14:16]==0`) |
| **Schema split** | born at **gen3** (gen2 = 3 keys, single `control_table`; gen3/gen4 = 4 keys, `control_fast` + `control_slow`) |
| **Row index source** | `opcode_table` entry: `fast_row = V & 0xFF`, `slow_row = V >> 8` |
| **Slow selector** | `slow_row == 0` ⇒ fast-only single step; `slow_row != 0` ⇒ run `control_slow[slow_row]` |
| **Slow grammar** | a packed stream of fixed-width steps; the default step body = `0x30c` (advance/NOP); tail byte = loop/step count (structure CERTAIN, semantics HIGH) |
| **Table-set deltas** | `saturate` = localized patch on the quantize/sparsity rows (143+); `transformer` = wholesale re-compaction |
...
> **NOTE — the slow word is *wider* than the fast word.** At gen4 the slow word carries ~48 bits more than the fast word (112 vs 64). Those extra `bytes[8:14]` are the iterative-program payload (loop body / per-step config) that a single-issue fast step does not need. This width asymmetry is the structural fingerprint of the partition: fast = a compact single-step config, slow = a wide iterative-program descriptor. gen2's fused single word sits at 96 bits, between the two.
...
|---|---|---|---|
| **A — lane-mode** | `[0:2]` u16 LE | 8 datapath lanes × 2 bits each; gates which lanes are live | HIGH |
| **B — step op-sel** | `[2]` | step control-op / ALU-op selector (5-bit op + high flag bit) | HIGH |
| **C — sequencer PC / imm** | `[4]` | per-step program counter / immediate (highest-entropy byte) | HIGH |
| **D — issue marker** | `[4:6]` | sequencer fetch/issue marker (`20 80`, `a0 8x` families) | HIGH |
| **E — operand / mux** | `[6:12]` | secondary operand / mux fields (low cardinality 4–9) | HIGH |
...
The 8 lanes here are the **8 datapath lanes** that the [2.27 datapath table](dve-datapath-table.md) configures (8 words per datapath block). The control word's lane-mode field gates which lanes are live for this step; the datapath block then configures the live lanes. The two tables are co-indexed by control row. Mode `01` = pass/default, `00` = disabled, `10`/`11` = alternate routes. The `0x5555 = 8×'01'` decomposition is solid; the exact silicon meaning of `10`/`11` is **SPECULATIVE**.

**Field B — step op-sel (`byte[2]`).** Distinct values are dominated by `0x10` (48 rows), with a `0x10..0x1f` step-variant cluster and a high-flag class (`0x80`/`0x83`, 4+1 rows). `byte[2] < 0x20` in 122 of 127 rows ⇒ a 5-bit op selector plus a high flag bit.

**Field C — sequencer PC/immediate (`byte[4]`).** 46 distinct values (the highest cardinality) — nearly contiguous `0..0x1f` plus scattered highs. This is the most data-like byte and climbs roughly monotonically with row index in the active region, consistent with a per-step program-counter / immediate.

> **GOTCHA — the gen2 field map does not transplant onto gen4 fast.** The offsets above (lane-mode `@[0:2]`, op-sel `@[2]`, PC `@[4]`, issue marker `@[4:6]`) describe the gen2 **12-byte** layout only; gen4 re-packs the word into 8 bytes with shifted fields. The gen2 step-valid marker `byte[6]=0x10` moves to `byte[4]=0x10` in gen4 fast (present in all 182 used fast rows), and gen2's lane-mode does not survive at `[0:2]` — gen4 fast `byte[0]` carries only 4 distinct values `{0,1,2,8}` while `byte[1]` becomes the 92-distinct operand/PC.

### 2.1 gen2 has no slow control table — only a proto-slow index

A targeted scan finds **zero** gen2 control rows carrying the `c3 30 0c` slow-program backbone that defines gen3/gen4 slow rows (§3). Every gen2 control row is fast-style microcode. The gen2 `opcode_table`'s high field (`V >> 7`, a "slow index" in [2.25](dve-opcode-table.md)) does **not** index a slow *control* table — there is none in gen2. The slow ops park on idle base control rows, and their iterative work is driven by a separate gen2 mechanism that is not shipped as a control blob. No `c3`/`30`/`0c` backbone appears in any gen2 row, across all five gen2 table-sets.
...
The slow word carries ~48 bits more than the fast word; the extra `bytes[8:14]` are the iterative payload.
...
The triplet `(12,0,3)` = **`0x30c`** repeats 3–4×. The slow row is a **packed micro-program**: a stream of fixed-width sequencer *steps*, each step's default body = `0x30c` (an advance/NOP step). The variation lives in (a) the **head nibbles** (`byte[0]`, low nibble of `byte[1]`), which substitute a non-default step body for the first step(s), and (b) the **tail byte** (`byte[6]`/`byte[7]`), which is the **step/loop count** — how many iterations the program runs (observed values `0,1,2,3,7,a,f`). The slow-only `bytes[8:14]` hold the loop body / bound config. The structure is byte-exact; the loop-count reading follows from the observed value set; the exact loop-bound encoding is **SPECULATIVE**.
...
So op 65 runs: the fast setup step (`control_fast[16]`, the `0x09` arith issue), then the slow program at `control_slow[48]` — a head step `0x001` followed by three `0x30c` advance steps, looped twice. A reimplementation that ignores `slow_row` and emits only the fast step under-issues every iterative op. The bytes are read directly; the head-step semantics beyond "non-default first step" are inferred from the pattern.
...
**HIGH:** gen2 field map (lane-mode `[0:2]`, op-sel `[2]`, PC `[4]`, issue marker `[4:6]`, operand/mux `[6:12]`); lane-mode gates the 8 datapath lanes; slow tail byte = loop/step count; fast/slow tier correlates with PerfSim's init-vs-NEP tier but is mechanistically independent.
