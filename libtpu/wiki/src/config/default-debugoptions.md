...
Proto3 carries no descriptor-level defaults — every scalar's wire default is the zero value (`false` / `0` / `""` / enum-0). The *effective* default is whatever this constructor stores on top of that zero state. The body therefore divides cleanly into two populations: the fields it **explicitly writes** (the non-zero seeds documented here), and the ~250 fields it **never touches**, which keep the proto-zero default established by the `DebugOptions::DebugOptions(this, 0)` base-construction call on entry (line 60). This page owns that explicit-write → value map. It does **not** own the field schema — field number, name, and type live on [`debugoptions-proto.md`](debugoptions-proto.md). It does not own the flag overrides ([`xla-flag-atlas.md`](xla-flag-atlas.md)) nor the AUTO/`-1`-sentinel resolution ([`autoproto-autoor-resolution.md`](autoproto-autoor-resolution.md)).
...
> **CORRECTION (DEFAULTS) —** the raw field-table report inferred that `xla_step_marker_location` (108) is "typically overridden to `STEP_MARK_AT_TOP_LEVEL_WHILE_LOOP` in DefaultDebugOptions (convention; not byte-confirmed)." The decompiled body of `DefaultDebugOptionsIgnoringFlags` at `0x1e66a860` contains **no** store to the step-marker field's offset — it is left at the proto-zero `STEP_MARK_AT_ENTRY=0`. Any non-zero step-marker default in this build comes from the *flag* layer or PJRT merge, not from this constructor. The two flag-wired fields `xla_tpu_detect_nan` (135, offset 580) and `xla_tpu_detect_inf` (136) are likewise **not** written here — both stay `false`, confirming the convention-FALSE inference by direct absence of a store.
