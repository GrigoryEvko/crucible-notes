...
This page supersedes the scattered mbarrier paragraphs in `codegen/atomic-warp-sreg-fence.md` (the 21-op printer table), `codegen/tcgen05-wgmma-mbarrier-cluster.md` (the finalize-phase fragment), `topics/cluster-sync-and-dsmem-handshake.md` (the transactional handshake), `dialects/cutlass/pipeline-and-tile-scheduler.md` (the producer/consumer step function), and `dialects/cutlass/seq-bar-and-block-striped.md` (the ring-of-slots view). Those pages now defer here for the mechanism itself.
...
- **Cluster transaction**. The cross-CTA variant. The producer maps the barrier into a peer CTA's distributed shared memory through `nvvm.mapa`, publishes `expected_txn` via `nvvm.mbarrier.txn` (the cluster-scope expect-tx op), then participates in `cluster.arrive` / `cluster.wait`. The DSMEM handshake on `topics/cluster-sync-and-dsmem-handshake.md` documents the rendezvous; the mbarrier state-machine view is just that the transaction byte count is published on a peer-CTA mbarrier rather than a local one.
...
[WGMMA Emission Protocol](wgmma-emission-protocol.md) is the consumer side of the TMA-transaction kind: the WGMMA wait-group sequence runs after `try_wait.parity` succeeds on the producer's mbarrier.
[tcgen05 Tensor Memory Model](tcgen05-tensor-memory-model.md) uses cluster-transaction mbarriers to coordinate the 2-CTA and 4-CTA TMEM staging copies that precede each MMA.
