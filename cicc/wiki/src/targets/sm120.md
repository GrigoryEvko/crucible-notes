...
The `sm_101a`/`sm_101f` rows are latent: cicc still carries the version codes and the
target strings, but ptxas 13.1.115 rejects `sm_101` and `sm_101a` at the gpu-name
parser (`Value 'sm_101' is not defined for option 'gpu-name'`), as it does `sm_104`,
`sm_107` and `sm_130`. Only the codes with an accepted gpu-name are reachable from a
compilation.
...
sm_110 (Jetson Thor, for automotive and robotics SoCs) sits between sm_100 and sm_120 in the architecture numbering. Despite the higher SM number, sm_110 is architecturally a datacenter Blackwell derivative and **retains tcgen05/TMEM support** — the tcgen05 gate explicitly allows sm_110a (SmVersion 1101) and sm_110f (1102). It lacks sm_120's `.offset.bindless` and f16 texture features but has full tensor core parity with sm_100/sm_103.
