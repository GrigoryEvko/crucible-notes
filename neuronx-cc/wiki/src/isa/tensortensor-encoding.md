...
> **CORRECTION (prominent).** An earlier wave (J05/J06) treated `visitInstCopyPredicated @0x125eb20` as a single-opcode encoder and called the whole thing "CopyPredicated", assigning the `0xEA` byte to it. **That is wrong.** The `0xEA` path is the `SelectReduce` form (the fused masked-copy-with-reduce); the true `CopyPredicated` (`0x72`), `CastPredicated` (`0x99`) and `CopyPredicatedScalar` (`0xE8`) are the *other three* branches of the same body. The field positions for the `0xEA` path are corrected below (it is a **2-D** AP band at `+0x18`/`+0x30` with the ALU bytes at `+0x24`/`+0x25`, **not** the 3-D `s3d3_tt` band). See [§3](#3-visitinstcopypredicated--four-opcodes-one-body-the-select-family).
...
CONFIRMED: `objdump -s -j .rodata` at `0x1df5760` reads `0302050d 0e0e030f 0e0f0504 06070908 / 0a0b010c …`. The `qword_1DF59E0` size LUT reads `01000000.. 01000000.. 02000000.. … 04000000` (uint8 = 1, a 16-bit dtype = 2, fp32 = 4, a 64-bit dtype = 8), all 20 entries CONFIRMED.
...
|---|---|---|---|---|---|
| `+0x00` | 1 | opcode `0x41`/`0x51`/`0x8A`/`0x8E` | `setupHeader(opByte)` | `tensor_tensor_opcode` | CONFIRMED |
| `+0x0C` (lo nibble) | 4b | in0 dtype | `sub_120E650(in0.Dtype)` | `s3d3_tt_dtype` | CONFIRMED (`movzx [r15+0xc]` … `mov [r15+0xc] @0x1235a40`) |
| `+0x0C` (hi nibble) | 4b | in1 dtype | `sub_120E650(in1.Dtype)` (`16*`) | `s3d3_tt_dtype` | CONFIRMED (`mov [r15+0xc] @0x1235a62`) |
| `+0x0D` | 1 | out dtype | `sub_120E650(out.Dtype)` | `s3d3_tt_dtype` | CONFIRMED (`mov [r15+0xd] @0x1235a75`) |
| `+0x0E` | 1 | ALU-op wire byte | `sub_12039C0(*(int*)(inst+0xF0))` | `s3d3_tt_valid_op` | CONFIRMED (`mov [r15+0xe] @0x1235a85`) |
| `+0x0E` | 1 | int32 override `0xC4`(eng4)/`0xC5`(eng6) | when int32×3 & `inst+0x90==1` | — | CONFIRMED (`@0x1235c1a`/`@0x1235e28`) |
| `+0x0F` | 1 | `num_active_partitions` | `in0.firstAPPair.num` (`*(*(in0+80)+8)`) | `s3d3_tt_src_element_cnt_check` | STRONG (`mov [r15+0xf] @0x1235abe`) |
| `+0x10` | 16 | dst access pattern | `assignAccess<TENSOR3D>(out)` | — | CONFIRMED |
| `+0x20` | 16 | in0 access pattern | `assignAccess<TENSOR3D>(in0)` | — | CONFIRMED |
| `+0x30` | 16 | in1 access pattern | `assignAccess<TENSOR3D>(in1)` | — | CONFIRMED |

ASSERT: `in0+88 == 0` (no `APPairs`) → `__assert_fail "idx<size()"` `SmallVector.h:0x128`. ENGINE: relayed from `inst+0x90` (DVE = 5 canonical), no recompute. **BROADCAST**: a stride-0 `APPair` inside `in0`/`in1` is passed verbatim by `assignAccess` — there is no broadcast flag in the band (STRONG).
...
### 3.1 SelectReduce (`0xEA` = 234) — 2-D AP band ✱ CORRECTED

The fused predicated-copy + reduce form (`inst+0x140 != 0`). This is the path J05/J06 mislabelled as "CopyPredicated". It is a **2-D** band — a masked per-lane op is a 2-D operation.

| Offset | W | Field | Source | pybind check | Conf |
|---|---|---|---|---|---|
| `+0x18` | 16 | dst access pattern | `assignAccess<TENSOR2D>(out)` | — | CONFIRMED (`lea [r15+0x18] @0x125f944`) |
| `+0x30` | 16 | mask access pattern | `assignAccess<TENSOR2D>(arg0)` | — | CONFIRMED (`lea [r15+0x30] @0x125f97b`) |
| (packed) | 16 | src access pattern | `assignAccess<TENSOR2D>(src)` | — | STRONG (3rd 2-D AP; pointer register-packed, not a direct `lea`) |
| `+0x24` | 1 | copy/bypass ALU byte | `sub_12039C0(0) = 0x00` | `s_select_reduce_op` | CONFIRMED (`[r15+0x24] @0x125f995`) |
| `+0x25` | 1 | predicate/reduce ALU byte | `sub_12039C0(*(int*)(inst+0x140))` | `s_valid_select_reduce_op` | CONFIRMED (`[r15+0x25] @0x125f9b5`) |
| `+0x26` | 1 | `useFalseFill` flag | `*(u8)(inst+0x118)` | — | CONFIRMED (`[r15+0x26] @0x125f9f3`) |
| `+0x27` | 1+ | false-fill immediate | `sub_12051E0` `std::visit(fill)` | — | CONFIRMED (`lea [r15+0x27] @0x125f9d7`, 3-arm variant writer) |
| `+0x28` (lo) | 4b | mask dtype | `sub_120E650(mask.Dtype)` | `s3d3_copy_pred_src0_dtype` | CONFIRMED (`[r15+0x28] @0x125fb0a`) |
| `+0x28` (hi) | 4b | src dtype | `sub_120E650(src.Dtype)` (`16*`) | `s3d3_copy_cast_pred_src1_dst_dty` | CONFIRMED (`[r15+0x28] @0x125fb2f`) |
| `+0x29` | 1 | out dtype | `sub_120E650(out.Dtype)` | `s3d3_copy_cast_pred_src1_dst_dty` | CONFIRMED (`[r15+0x29] @0x125fb4e`) |
| `+0x2A` | 1 | `num_active_partitions` | `mask.firstAPPair.num` | `s3d3_copy..src_element` | STRONG (`[r15+0x2a] @0x125fb75`) |
| `+0x2B` | 1 | false-fill dtype | `sub_120E650(fill.Dtype)` | — | CONFIRMED (`[r15+0x2b] @0x125fb42`) |
| `+0x2C` | 1 | EngineAccumulationType byte | `sub_12038D0(*(int*)(inst+0x144))` | `s_zero_accum_cmd` | CONFIRMED (`[r15+0x2c] @0x125fb8d`) |

VARIANT GUARD (CONFIRMED): `inst+0x138 == 0` else `__throw_bad_variant_access`. The `std::visit` (`sub_12051E0`) routes the false-fill `Argument` variant `{PhysicalAP / RegisterAP / ImmediateValue}` to one of three writer lambdas, each storing the scalar/addr into `+0x27`. ENGINE DVE(5). COLLECT inserts opcode word 234.

### 3.2 CopyPredicated (`0x72`) / CastPredicated (`0x99`) — 3-D `s3d3_tt` band

Guard: `inst+0x140 == 0` and `src` is a TENSOR. **Opcode byte** (CONFIRMED): `(out.dtype == src.dtype) ? 0x72 : 0x99` — the same cast-bit trick as TensorCopy `0x46`/`0x47` (§4), lifted to the predicated family. `mov eax,0x72 @0x125ecd2` / `mov eax,0x99 @0x125ecc3`. This branch reuses the **TensorTensor** `s3d3_tt` control band byte-for-byte, with the mask AP in the in1 slot:

| Offset | W | Field | Source | Conf |
|---|---|---|---|---|
| `+0x0C` (lo) | 4b | mask dtype | `sub_120E650(mask.Dtype)` | CONFIRMED (`[r15+0xc] @0x125f3c6`) |
| `+0x0C` (hi) | 4b | src dtype | `sub_120E650(src.Dtype)` (`16*`) | CONFIRMED (`[r15+0xc] @0x125f3e2`) |
| `+0x0D` | 1 | out dtype | `sub_120E650(out.Dtype)` | CONFIRMED (`[r15+0xd] @0x125f3fc`) |
| `+0x0F` | 1 | `num_active_partitions` | `mask.firstAPPair.num` | STRONG (`[r15+0xf] @0x125f41c`) |
| `+0x10` | 16 | dst access pattern | `assignAccess<TENSOR3D>(out)` | CONFIRMED (`lea [r15+0x10] @0x125f40f`) |
| `+0x20` | 16 | src/true access pattern | `assignAccess<TENSOR3D>(src)` | CONFIRMED (`lea [r15+0x20] @0x125f437`) |
| `+0x30` | 16 | mask access pattern | `assignAccess<TENSOR3D>(arg0)` | CONFIRMED (`lea [r15+0x30] @0x125f448`) |
...
| Offset | W | Field | Source | pybind check | Conf |
|---|---|---|---|---|---|
| `+0x10` | 16 | dst access pattern | `assignAccess<TENSOR3D>(out)` | — | CONFIRMED (`lea [r15+0x10] @0x125fe53`) |
| `+0x30` | 16 | mask access pattern | `assignAccess<TENSOR3D>(arg0)` | — | CONFIRMED (`lea [r15+0x30] @0x125fe61`) |
| `+0x20` | 1 | mask dtype | `sub_120E650(mask.Dtype)` | `d3_cp_pred_scalar..` | CONFIRMED (`[r15+0x20] @0x125fd4e`) |
| `+0x21` | 1 | `useFalseFill` flag | `*(u8)(inst+0x118)` | — | CONFIRMED (`[r15+0x21] @0x125fcf3`) |
| `+0x22` | 1 | `num_active_partitions` | `out.firstAPPair.num` | — | STRONG (`[r15+0x22] @0x125fd0a`) |
| `+0x29` | 1 | src/scalar dtype | `sub_120E650(src.Dtype)` | `d3_cp_pred_scalar..` | CONFIRMED (`[r15+0x29] @0x125fd17`) |
| `+0x2A` | 1 | out dtype | `sub_120E650(out.Dtype)` | `d3_cp_pred_scalar..` | CONFIRMED (`[r15+0x2a] @0x125fd2a`) |
| `+0x2B` | 1+ | scalar true-value immediate | `sub_12051E0` `std::visit(src)` | — | CONFIRMED (`lea [r15+0x2b] @0x125fd3d`) |
...
**Opcode byte** (CONFIRMED, the cast-bit trick): `opcode = (src.Dtype@+0x30 != dst.Dtype@+0x30) + 0x46`, i.e. `0x46` (same-dtype plain copy) / `0x47` (casting copy). Disassembled: `cmp eax,[r13+0x30] @0x1237f92`; `setne r15b @0x1237f96`; `add r15d,0x46 @0x1237f9a`.

This is the **Family-C** 4-D copy band (bundle base register `rbx`):

| Offset | W | Field | Source | pybind | Conf |
|---|---|---|---|---|---|
| `+0x00` | 1 | opcode `0x46`/`0x47` | `(srcDty != dstDty) + 0x46` | `s_copy_opcode` | CONFIRMED |
| `+0x0C` | 20 | SRC mem-pattern | `assignAccess<TENSOR4D>(arg0)` | — | CONFIRMED (`lea [rbx+0xC]`) |
| `+0x20` | 1 | SRC dtype (wire tag) | `sub_120E650(getReinterpretedCopyDtype(src.Dtype))` | `s3d3_copy..` | CONFIRMED (`[rbx+0x20]`) |
| `+0x21` | 1 | DST dtype (wire tag) | `sub_120E650(getReinterpretedCopyDtype(dst.Dtype))` | `s3d3_copy..` | CONFIRMED (`[rbx+0x21]`) |
| `+0x22` | 1 | num channels / partition count | `arg0.MemLoc.partn_count` (`*(*(arg0+0x50)+8)`) | `copy_s_..active_channels` | CONFIRMED (`[rbx+0x22]`, guarded `arg0+0x58 != 0`) |
| `+0x25` | 1 | reserved/flag = 0 | const 0 | — | CONFIRMED (`[rbx+0x25]`) |
| `+0x2C` | 20 | DST mem-pattern | `assignAccess<TENSOR4D>(out)` | — | CONFIRMED (`lea [rbx+0x2C]`) |
...
| Offset | W | Field | Source | pybind | Conf |
|---|---|---|---|---|---|
| `+0x00` | 1 | opcode `0x49`/`0x4D` | `mode@inst+0xF0 → setupHeader` | `memset_opcode` | CONFIRMED |
| `+0x1C` | 4 | `dst_element_count` (DWORD) | `getNumElementsPerPartition(out)` (`sub_1250E50`) | `instr.dst_element_count` | CONFIRMED (`lea [r13+0x1c] @0x125b45e`) |
| `+0x20` | 1 | dtype (wire tag) | `sub_120E650(getReinterpretedCopyDtype())` | — | CONFIRMED (`[r13+0x20] @0x125b41d`) |
| `+0x22` | 1 | `num_active_partitions` | `out.firstAPPair.num` | — | STRONG (`[r13+0x22] @0x125b435`) |
| `+0x28` | 4 | FILL VALUE (DWORD immediate) | `*(u32)(inst+0xF8)` | `memset_set_value` | CONFIRMED (`DWORD [r13+0x28] @0x125b455`) |
| `+0x2C` | 20 | output access pattern | `assignAccess<TENSOR4D>(out)` | — | CONFIRMED (`lea [r13+0x2c] @0x125b47d`) |
...
> **CORRECTION — MoveShape is not a `visitInst*` method.** It is `CoreV2GenImpl::generateMoveShape(bir::InstDMA&, bir::AccessPattern const*)` — a **DMA-family helper**, called from the `InstDMA` codegen path with `a2 = InstDMA`, `a3 = the AccessPattern` whose shape is loaded. It is not a tensor-tensor op; it programs the DVE "shape registers" with a per-dim `(step, num)` pattern. Opcode `0xB2` (178).

GUARD up front (CONFIRMED): `reportError "Hardware Restriction: Shape registers doesn't work with TensorIndirect AP"` — MoveShape rejects indirect APs. Pattern build: `rank = AP+88` (the `APPair` count); ASSERT `rank ≤ 5` else `EC1124 NeuronAssertion "ImmPattern.size() <= SHAPE_REGISTER_SIZE + 1"`. For each dim `step[i] = DescGenHelper::getAPStepOnHW(...)`, `num[i] = AP.pairs[rank-i].num`; the remaining dims up to 4 pad out with `step = qword_1DF59E0[out.dtype]` (the default contiguous step = element byte size) and `num = 1`.

| Offset | W | Field | Source | pybind | Conf |
|---|---|---|---|---|---|
| `+0x00` | 1 | opcode `0xB2` | `setupHeader` | `move_opcode` | CONFIRMED |
| `+0x0C` | 2 | const `0x0100` (256) | literal | — | CONFIRMED |
| `+0x0E` | 1 | flag = 1 | literal | — | CONFIRMED (`mov BYTE [r14+0xe],0x1 @0x1213f64`) |
| `+0x10` | 1 | mode/replication byte | `*(u8)(a3+44)` | — | CONFIRMED |
| `+0x20` | 16 | 4× step DWORD (per-dim HW step) | `getAPStepOnHW` / dtype-size default | — | CONFIRMED (`movups [rax+0x20] @0x1213f48`) |
| `+0x30` | 16 | 4× num WORD (per-dim count) | AP pair count, default 1 | — | CONFIRMED (`movups [rax+0x30] @0x1213f4c`) |
...
**Select (IT51)** has **no** `CoreV{2,3,4}GenImpl::visitInstSelect` (CONFIRMED — `nm -DC | rg -c 'CoreV[234]GenImpl::visitInstSelect'` = 0). `out = preds ? onTrue : onFalse` is realised by `lower_select`: `Select(IT51) → InstGenericCopy(out←onFalse seed) + InstCopyPredicated(IT52)`. The IT52 leaf is §3's visitor, whose operand profile picks the wire opcode: scalar onTrue → `0xE8`; tensor same-dtype → `0x72`; tensor cast → `0x99`; reduce-op present → `0xEA`. A `Select` never reaches a bundle — it would be a hard codegen error the schedule prevents. (The only `visitInstSelect` bodies are in `birverifier`, `birsim`, `SimplifyMemset`, the `SplitSelectVisitor` lower pass, and `ConstantPropagate` — none are codegen emitters.)

**TensorDequantize** (ISA opcode `0x7B` = 123) has **no** `CoreV*` encoder symbol (CONFIRMED absence). The dequant is lowered to a scale-multiply / `TensorScalar` chain before CoreV2 codegen; there is no leaf bundle emitter (STRONG on the lowering rationale).

---

## Confidence ledger

**CONFIRMED** (direct store/cmp/opcode disassembled byte-exact this task):
