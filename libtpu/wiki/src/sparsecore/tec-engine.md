...
P-3-303 hypothesized that on the TAC-bearing gens the outliner splits a tile_task into an `"access"` (TAC) function plus an `"execute"` (TEC) function. Byte tracing shows **no such split exists in this wheel on any generation** — the SC-MLO compiler is a 2-sequencer SCS+TEC pipeline (VF, GL, *and* GF). This is the single most consequential fact about where the TEC sits in the pipeline.
