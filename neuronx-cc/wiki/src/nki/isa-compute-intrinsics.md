...
> **CORRECTION (W01-VERIFY) —** the `@0x…` offsets for the validators and the Cython interned-name dispatch derive from the `neuron_isa.cpython-3XX.so` decompilation cited in the backing report (D-W01). The `.so` and its IDA sidecar are not present in the standard `decompiled/`/`ida/` trees of this corpus snapshot, so those offsets are carried at the report's stated CONFIRMED grade rather than re-derived here. Every *rule*, *signature*, *enum*, and *dtype set* on this page is re-verified directly against the shipped `neuronxcc-stubs/nki/isa/__init__.pyi`, which is binary-distributed and therefore primary evidence.
