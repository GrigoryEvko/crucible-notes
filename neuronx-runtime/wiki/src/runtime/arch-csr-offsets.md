...
> **CORRECTION —** an earlier revision of the front-matter line above claimed `.data` carries a `+0x400000` VMA→file-offset delta. That is **wrong** for `libnrt.so`. `readelf -lW` shows the RW `LOAD` segment as `LOAD 0xbeeaa0 0x0000000000beeaa0 0x0000000000beeaa0 ... RW` (Offset == VirtAddr == `0xbeeaa0`, delta **zero**), and `readelf -SW` shows `.data` at `Address 0xc07e00 / Off c07e00` — identical. The `0x400000` delta belongs to a *different* binary (the libtpu / Kaena-profiler image), not libnrt — read `.data` globals at their VMA directly.
...
The DVE table offsets/sizes and the DVE parameter-RAM base differ between SUNDA and CAYMAN/MARIANA. SUNDA has no DVE table-*offset* getter (it returns only *sizes* — offsets are implicit) but **does** expose a DVE parameter-RAM getter (`get_dve_parameter_ram_params_sunda @0x478d50`); CAYMAN and MARIANA expose both, and — contrary to an earlier reading — their table-offset quads are **byte-identical** to each other (see the §3 CORRECTION below).
...
| DVE param-RAM base + size | `get_dve_parameter_ram_params_<arch>` · sun `0x478d50` / cay `0x47ac70` / mar `0x477400` | `0x2B84000`, `0x400` | `0x2B87000`, `0x400` | `0x2B87000`, `0x400` | CERTAIN |
| DVE table offsets (4 out) | `get_dve_table_offsets_<arch>` · cay `0x47ac80` / mar `0x477410` | *(none — sizes only)* | `0x2B82000`,`0x2B80000`,`0x2B81000`,`0x2B86000` | `0x2B82000`,`0x2B80000`,`0x2B81000`,`0x2B86000` | CERTAIN |
...
> **NOTE —** the DVE table-offset prefix `0x2B8xxxx` aligns with the DVE engine sequencer aperture low word (`0x802B00000` → `0x2B00000`), so the four DVE tables and the param-RAM live inside the DVE tile's local window. CAYMAN and MARIANA emit **byte-identical** DVE table-offset quads *and* sizes *and* param-RAM bases — `get_dve_table_offsets_mariana @0x477410` and `_cayman @0x47ac80` have identical bodies, as do the two param-RAM getters (`@0x477400`/`@0x47ac70` both `0x2B87000`). A reimplementer can safely share the Trn2 == Trn3 DVE offset/size/param-RAM constants; only SUNDA (param-RAM `0x2B84000`, no offset getter) diverges. The DWARF `dve_config_dma_state_t` member order maps the four offsets to opcode / control-fast / control-slow / datapath (name↔slot pairing MED; offsets/sizes HIGH).

> **CORRECTION —** earlier revisions of this page listed the MARIANA DVE **table offsets** as `0x2B7B000`,`0x2B79000`,`0x2B7A000`,`0x2B7F000` and the MARIANA DVE **param-RAM** base as `0x2B80000` (with the SUNDA param-RAM cell left blank). All three were wrong. The four `0x2B7x000` table offsets are **fabricated** — they appear nowhere in the binary (`objdump -d libnrt.so | rg '2B7B000|2B79000|2B7A000|2B7F000'` → empty). The true MARIANA values are byte-identical to CAYMAN's: `objdump -d --start-address=0x477410 aws_hal_get_dve_table_offsets_mariana` → `movq $0x2b82000,(%rdi); $0x2b80000,(%rsi); $0x2b81000,(%rdx); $0x2b86000,(%rcx)` (matching `@0x47ac80 _cayman` immediate-for-immediate). For param-RAM, `objdump -d @0x477400 _mariana` → `movq $0x2b87000,(%rdi)` (== CAYMAN's `@0x47ac70`, **not** `0x2B80000`), and `@0x478d50 _sunda` → `movq $0x2b84000,(%rdi)`; the corrected MARIANA `0x2B87000` now agrees with sibling [arch-stpb](arch-stpb.md) (which already stated `0x2B87000`).
