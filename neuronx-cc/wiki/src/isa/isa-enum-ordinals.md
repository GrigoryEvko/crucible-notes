...
The ordinals are grounded from three classes of in-binary evidence, every one re-walked this pass: **(E1)** name-decode switches — `enum_variant_string_<enum>(int,char*,int)` or `klr::to_string(<enum>&)`, a dense `case N → strncpy("Name")` switch where `case N == ordinal N` and the literal is the wire mnemonic; **(E2)** BIR-enum→wire-byte `convert<…>` encoders, each tagged by its `"Invalid enum variant for enum <Name>"` error string, whose per-case `mov eax,IMM` or `movzx eax, byte[LUT+idx]` is the wire value; **(E3)** the `.rodata` wire LUTs themselves (`byte_1DFBAD0` dtype, `unk_1DF5790` collective, `byte_1DF5780` branch-compare, `byte_1DF577A` wait-mode). The spine — the dtype LUT, the AluOp ordinals, the comp_op set, the CollectiveKind LUT, the EngineType map — comes from encoder writes and LUT bytes read byte-exactly, and agrees with the sibling encoding pages that consume them.

## At a glance — the 21-enum roster

| # | Enum (`NEURON_ISA_TPB_*` / `bir::` / `klr::`) | Cardinality | Form | L1=L3? | Consumed by | Conf |
|---|---|---|---|---|---|---|
| 1 | `_OPCODE` (`bir::InstructionType`) | 161 live (0..255) | name-decode (E1) | n/a | every encoder | CERTAIN |
| 2 | `_ALU_OP` (`bir::AluOpType`) | 30 (V2/3) / 33 (V4) | convert (E2) | [2.14](tensorscalar-encoding.md), [2.12](pool-reduce-encoding.md) | ✗ remap | CERTAIN |
| 3 | `_DTYPE` (`bir::Dtype`) | 20 | LUT (E3) | all operands | ✗ full remap | CERTAIN |
| 4 | `_ACCUM_CMD` (`klr::AccumCmd`) | 5 + reserved | name-decode (E1) | PE / Act | ✗ (wire reorder) | CERTAIN |
| 5 | `_DGE_COMPUTE_OP` (`klr::DgeComputeOp`) | 2 + reserved | name-decode (E1) | DMA/DGE | = | CERTAIN |
| 6 | `_CCE_OP` (= `AluOpType` field) | subset `{0,4}` | field identity (E2) | collective copy | = (uses #2) | CERTAIN |
| 7 | `_PERF_OPT_MODE` (`klr::MatmulPerfMode`) | 4 | identity convert (E2) | PE matmul | ✗ (klr≠BIR) | CERTAIN |
| 8 | `_TENSOR_SUBDIM` (`klr::TensorSubDim`) | 4 + reserved | name-decode (E1) | reduce/bcast | (shifted +1) | CERTAIN |
| 9 | `_DROPOUT_THRESHOLD_TYPE` (`klr::`) | 2 + reserved | name-decode (E1) | Dropout | — | CERTAIN |
| 10 | `_TENS_SCALAR_REV_OPS` (`klr::`) | 4 + reserved | name-decode (E1) | TensorScalar | — | CERTAIN |
| 11 | `_INDIRECT_DIM` (`bir::IndirectDim`) | 4 | 2string + emit name | gather/indirect | = | HIGH |
| 12 | `_COLLECTIVE_TYPE` (`bir::CollectiveKind`) | 11 (2..10 wired) | LUT (E3) | [2.22](collective-customop-encoding.md) | ✗ reorder | CERTAIN |
| 13 | `_WAIT_MODE` (`bir::sync::Wait`) | 3 | LUT (E3) | [2.20](sp-sync-encoding.md) | ✗ remap | CERTAIN |
| 14 | `_UPDATE_MODE` (`bir::sync::Update`) | 2+ | identity write (E2) | [2.20](sp-sync-encoding.md) | = (raw) | CERTAIN |
| 15 | `_PE_FP32MODE` (PE precision sig.) | `{0,2}` | byte pair (E2) | [2.10](pe-matmul-encoding.md) | n/a | HIGH |
| 16 | `_RAND_ALGORITHM` (`bir::RandomAlgorithmKind`) | 3 | 2string, identity wire | [2.16](rng-encoding.md) | = | HIGH |
| 17 | `_MATMUL_PSUM_ACCUMULATE_FLAGS` | 3-bit field | bit setters (E2) | [2.10](pe-matmul-encoding.md) | bits | CERTAIN |
| 18 | `_MATMUL_ZERO_REGION` (MX window) | range field | field role (E2) | [2.10](pe-matmul-encoding.md) | n/a | CERTAIN |
...
>
> **GOTCHA — CoreV2 and CoreV4 share one ALU-op band.** There is no "compressed" CoreV2 comparison scheme: `sub_12039C0` emits the *same* bytes as the CoreV4 band for ordinals `0..29` (`not_equal=19→0x18`, `is_gt=20→0x13`, `is_ge=21→0x14`, `is_lt=22→0x16`, `is_le=23→0x15`, `rsqrt=28→0x1D`, `abs=29→0x19`). CoreV4 only *adds* `30/31/32`; the dedicated `24/25` slots are shared. See also [2.14](tensorscalar-encoding.md).
...
Member **names** from `bir::Dtype` 2string (`libBIR @0x2641e0`). The wire bytes are exact; the member names are joined by ordinal from the 2string table. This is a **full remap** — practically no entry equals its L1 ordinal.
...
**Grounding (E1):** `klr::to_string(klr::AccumCmd&) @0xf7f630` — a 6-case switch (`cmp [rsi],5; ja default`, jump-table base `0x1DEF348`). This is the **KLIR text-IR** enum; the **libBIR wire** form is a *separate* space (`EngineAccumulationType`, see the wire row below).
...
**ENUM 6 (`_CCE_OP`):** not a distinct enum — it is the `AluOpType` returned by `InstDMACopy::getCceOp()`, encoded by the ENUM 2 converter. Shipped legalization admits only `{bypass=0x00 (plain copy), add=0x04 (reduce-on-copy)}`. **Use the ENUM 2 table.** The field identity is exact; the admitted value set is read from the shipped legalization.

---

## ENUM 7 — `_PERF_OPT_MODE` (`klr::MatmulPerfMode`)

**Grounding (E1+E2):** `klr::to_string(klr::MatmulPerfMode&) @0xf7f030` (`cmp eax,1/2/3`); the CoreV2/3 wire encoder `convert sub_1203630 @0x1203630` (`"Invalid enum variant for enum MatmultPerfMode"`) is **identity** `mov eax,edi` for `edi∈{0,1,2,3}` (`edi>3 → ERR`).
...
**Grounding (E1):** `klr::to_string(klr::TensorSubDim&) @0xf7fe30`.
...
**Grounding (E1):** `klr::to_string(klr::DropoutThresholdType&) @0xf7f5b0`.
...
**Grounding (E1):** `klr::to_string(klr::TensorScalarReverseOps&) @0xf7fd90`.
...
**Grounding:** `bir::IndirectDim` 2string (`libBIR @0x4021a0`); libwalrus emits the dim **name** via `convertIndirectDim sub_1204220 @0x1204220` and gates legality via `valid_indirect_dim_by_mode` (`@0x1DD4B70`). The dim ordinal *is* the BIR ordinal (no remap).
...
Related — `klr::IndexMissBehavior @0xf82fc0` (the indirect-copy miss handler, *not* the dim): `0`=reserved/UNABLE, `1`=`Imm` (fill with immediate), `2`=`Skip` (skip the miss) — `@0xf83010`/`@0xf83020`.
...
Member **names** from `bir::CollectiveKind` 2string (`libBIR @0x4016c0`). The wire bytes are exact; the member names are joined by ordinal from the 2string table.
...
**Grounding (E2):** same encoder (`visitInstEventSemaphore @0x1217df0`). Unlike wait-mode, the update mode is written **raw / identity, no LUT** (`call Update::getMode @0x12180ed`; `mov [r14+0x22],al @0x12180f9` — update byte at `bundle+0x22`). The mode ordinal *is* the wire byte.
...
**Grounding (E2):** FP32 matmul uses a two-pass 2-FMA lowering; the precision mode rides `bundle+0x20`/`+0x21`. For `fp32r` (Dtype 17) the encoder writes the 16-bit signature `*(u16)(bundle+0x20) = 0x020A` ([2.10](pe-matmul-encoding.md) `@0x1258f16`/`@0x124972d`): byte `+0x20 = 0x0A` (the `float32` wire dtype tag, ENUM 3) and byte `+0x21 = 0x02` (the FP32MODE selector). Plain fp32 (Dtype 16) uses the standard 2-FMA without the mode byte. The wire bytes are exact; [INFERRED] the discrete member set, which is not name-decoded anywhere.

| fp32 mode | byte `+0x21` | dtype `+0x20` | grounding |
|---|---|---|---|
| standard fp32 (Dtype 16) | `0x00` | `0x0A` | no mode byte |
| fp32r 2-FMA round (Dtype 17) | `0x02` | `0x0A` | `0x020A` signature |
...
**Grounding:** `bir::RandomAlgorithmKind2string @0x401de0` (libBIR) + inverse `string2 @0x40ef50`, round-trip verified; the simulator's `visitInstRand @0x1d6290` dispatches on this ordinal (`mov esi,[rdi+0xF0]`). The ISA `random_algorithm` field on `Rng` (opcode 118) carries the ordinal **identity** (no remap on this path). The ordinals come from the 2string table and the wire is identity. [2.16](rng-encoding.md) carries the same map.
...
> **GOTCHA — the algorithm ordinals start at `LFSR`, not `PCG32`.** The order is `LFSR=0, PCG32=1, PHILOX_1=2`, per the `2string` `@0x401de0` / `string2` `@0x40ef50` decode. The companion `RandomDistributionKind` (the `Rng` `distribution` field, identity wire) is `{0:Raw, 1:Uniform, 2:Normal, 3:Binomial}`. The gen-2 `Rand2` path runs a fixed `XORWOW` over a *separate* state vector and namespace — a different enum space entirely ([2.16](rng-encoding.md)).

---

## ENUM 17 — `_MATMUL_PSUM_ACCUMULATE_FLAGS` (bundle `+0x2B`, a 3-bit field)

**Grounding (E2):** not a scalar enum — a 3-bit flag byte at `bundle+0x2B`, OR-ed by the calc-chain setters (CoreV2 `generateMatMul` inline `@0x1248650`; CoreV3 `generateMatMulAccumulateFlag @0x1428630`). Init `0x00` (`mov byte[r15+0x2b],0 @0x12493b3`). The masks are exact; the valid-set is read from the setters.
...
**Grounding (E2):** on CoreV4 MX, `generateMatmultMx` writes a PSUM zero-region descriptor at `bundle+0x2F` copied from `inst+0x118` (present-tag `inst+0x138`). It is the pow2-aligned PSUM **bank range** the START matmul clears (`get_psum_zero_region_bank_range`), not a small value-enum.
...
**Grounding (E2, validation only):** `NEURON_ISA_TPB_IMM_SRC` appears only as a validator argument (`has_valid_immediates @0x127ff90`, `ts_addr_immediate_check @0x12c34c0`). No name-decode / serializer exists. The validator branches on `src∈{0,1,2}`: `test sil,sil` (0), `cmp sil,1`, `cmp sil,2` (`@0x127ff9f`/`@0x127ffa7`). The 3-value cardinality is exact; [INFERRED] the member names, since no name table exists.
...
Not a wire field of these instruction bundles, but the discriminant every per-engine map keys on, and the one ordinal a reimplementer needs alongside the opcode. Byte-pinned from `bir::EngineType2string` (`libBIR @0x47fa80`) + `bir::EngineType2ExternalName` (`@0x47fca0`), cross-confirmed by `getEngineCount` (`@0x47d820`) and `getValidEngines` ([1.01 the arch object model](../arch/arch-object-model.md)).
...
Each of the page's central LUTs is read from `.rodata` and agrees with the encoding page that consumes it:

| LUT | Bytes | Consumer that agrees |
|---|---|---|
| Dtype wire tags `byte_1DF5760` | `03 02 05 0d 0e 0e 03 0f 0e 0f 05 04 06 07 09 08 0a 0b 01 0c` | [2.10](pe-matmul-encoding.md) (`b[0x20] = byte_1DF5760[dtype]`, fp32r→`0x0B`, fp8×4→`0x0E`/`0x0F`). The sole core_v4 divergence at index 2 (`0x10` vs `0x05`) lines up with CoreV3 *rejecting* the ×4-packed dtypes |
| AluOpType ordinals | `19→0x18, 20→0x13, 21→0x14, 22→0x16, 23→0x15, 28→0x1D, 29→0x19`, plus v4 `30→0x20, 31→0x21, 32→0xC8` | the jump-table arms in [2.14](tensorscalar-encoding.md) §`sub_12039C0` |
| comp_op set `unk_1DF5780` | `01 02 03 04 05 06 09 0a 0b 0c 0d 0e` | [2.20](sp-sync-encoding.md) (IMM `1..6` / REG `9..14`, gap at wire 7/8) |
| CollectiveKind `unk_1DF5790` | `01 02 03 04 09 05 06 07 08` (`AllToAllV=0x09` out of sequence) | [2.22](collective-customop-encoding.md), objdump-verified there for `kind ∈ {2..10}` |
| EngineType map | `PE=3, Pool=1, Activation=2, DVE=5, SP=6, DMA=4`, `Unassigned=0`, `ALL=7` | [1.01](../arch/arch-object-model.md), across `EngineType2string`, `getEngineCount`, and `getValidEngines` |

Two entries in the roster are not pinnable from these binaries: `_DGE_OPCODE` has no enum at all, and the `_IMM_SRC` member names appear only in validator strings. Both are marked [INFERRED] in place.
