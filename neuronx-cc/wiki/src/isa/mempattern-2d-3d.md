...
> **NOTE — unused free dims are `{stride=1, num=1}`-filled, never zeroed.** A descriptor template wider than the access pattern's active dimension count fills the slack with the `0x00010001` immediate (`stride=1`, `num=1`), so a degenerate dimension iterates exactly once rather than zero times. A `num=0` word reads as a decode error, not a no-op. The unit-fill is emitted by the shared packer (§3); see also [2.3](tensor-descriptors.md).
...
⇒ `slot_size(N) = 4 + 4N`. `assignAccess<MEM_PATTERN3D>` @ `0x15092b0` loads the 3D string with `lea rsi,[rip+…] = 0x1d6e920` at `0x1509551` — the identical string the `TENSOR3D` dispatcher cites. The "mem pattern" wording even appears in the *TENSOR* asserts, because the string table is shared; that sharing is itself the byte-twin proof.
...
The byte **width** is the same (12/16/20); the C++ type is nominally distinct so the op-encoder can route each operand to the right validator by role.

> **GOTCHA — `mem2d_valid` / `mem3d_valid` are not "plain-tensor SRC" validators.** Their declared input type is `MEM_PATTERN2D/3D`, the DST/general mem-pattern type. They do serve src operands as well (called with `WRITE_TENSOR=0`), but the type attribution is `MEM_PATTERN`, not `TENSOR` — the byte decode is unaffected either way.
...
The semantics: the **DST** (`MEM_PATTERN`) is a *union member* of the 64-byte `INST_UNION` compute bundle — its dispatcher asserts the union's field-presence tag is set (the dst slot is present). The **SRC** (`TENSOR`) is a standalone static read pattern — its dispatcher asserts the access pattern is **not** indirect. This is the cleanest in-binary expression of the operand-role split. Both dispatchers then run the **same** "must be SB or PSUM" location assert (`isLocationSB` @ `0x62a2c0` `||` `isLocationPSUM` @ `0x606230`) and route through the same vtable slot.

The two asserts do not swap: the "static tensor pattern but has indirect field" wording is the **SRC** dispatcher's, and the union field-presence check is the **DST** dispatcher's.

*Anchors: `0x1d71d18` for `MEM_PATTERN3D`, `0x1d71d58` the same wording for `MEM_PATTERN2D`, `0x1d71dc8` for the `TENSOR` (SRC) dispatcher.*
...
The validator is one function; the role is the flag vector.
...
The shared `assignAccess3D` leaf (`0x150ccf0`) additionally carries an inline indirect/gather arm (two `ADDR4` words: data base @+0, index @+4, stamping `desc[+3] |= 0x20`) reached for an *indirect* dst/src; the plain static DST path skips it. SRC and DST differ **only** in the wire-struct type and the validator they bind — not in a single packed byte. The vtable-slot loads and the total absence of any `MEM_PATTERN` packer symbol are what pin that.
...
The encoder folds the bank into the address upstream as `(part<<15) + (bank<<11) + inPartAddr` — the bank stride is `2048 B = 0x800`, i.e. `bank<<11`, matching the constant PSUM bank size (`getPsumBankSize = 2048`, [1.05](../arch/sbuf-psum-geometry.md)). The descriptor carries the bank *only* as part of its start address; the packer writes no separate bank field (window math in [2.2](addr4.md), packer in §3).
...
| `+0x23` / `+0x28` | PSUM-dst dtype | LUT[BIR Dtype]; selects the dst validator path (§5.3). **Per-generation**: dense matmul (op `0x02`, CoreV2/V3) at `+0x23`; MX matmul (op `0x100A`, CoreV4) at `+0x28`. See §6. |
...
These match the PE-ISA accumulate cluster in [2.9](neuron-isa-tpb-capstone.md).
...
```c
// mem3d_valid @0x14467d0 — full body
...
```c
// mm_dst_mem3d_valid_nc_v4 @0x1447fe0 — full body 0x1447fe0..0x144814c
...
*Anchors: body `0x1447fe0..0x144814c`; dtype helpers `0x142de60` (`{8..11}`) and `0x142de70` (`{4..7}`); the window test `sub 0x2000000` / `cmp 0x3FFFFF` at `0x1448053` / `0x144805a`.*
...
The sparse-MX matmul `is_valid_smx1d3_mm` @ `0x1501ac0` likewise calls `mm_dst_mem3d_valid_nc_v4(…, dt=6)` for its bf16 PSUM dst (`0x1501739`). The `union+0x28` / `union+0x30` slots match [2.9](neuron-isa-tpb-capstone.md).
...
`mem2d_valid`: 12 B = `rdi` (bytes +0..7) + `esi` (bytes +8..0xB). PSUM path: `tensor_start_addr_valid(addr@+0, dt=9, WR=1, PSUM=1, SBUF=0)`; `testb [stride1 hi], 0xe0`; tail-validates the `@+4` word. SBUF path: `addr@+0` valid; `(byte3 & 0x70) != 0x40`; `num0@+8 ≠0 ∧ num1@+0xA ≠0` (num tests at `0x1446998` / `0x14469a1`).

`mem4d_valid`: byte3 read at `[rsp+0x33]`; PSUM stride check at `[rsp+0x38]`; nums at `[rsp+0x3c/+0x3e/+0x40/+0x42]` = `num0..3 @+0xC/+0xE/+0x10/+0x12` — confirms the 20-B layout. `m4d_not_in_psum` @ `0x14463a0` is a 4D dst that asserts its secondary/indirect word is **not** in the PSUM window — a DST-PSUM-exclusion guard for the 4D ops.
...
|---|---|---|---|---|
| `+0x00` | *(bundle header / opcode)* | — | instruction word | CERTAIN |
| `+0x10` | **ifmap (moving) SRC** | `TENSOR3D` (16 B) | read, SBUF-resident | CERTAIN |
| `+0x14` | ifmap dtype | byte | SRC dtype | CERTAIN |
| `+0x23` | **PSUM-DST dtype (dense)** | byte | dense matmul op `0x02`, CoreV2/V3; selects dst validator (`==6` → `nc` path) | CERTAIN |
| `+0x28` | **PSUM-DST dtype (MX)** | byte | MX matmul op `0x100A`, CoreV4; selects dst validator (`==6` → `nc` path) | CERTAIN |
| `+0x2B` | **accumulate** | byte | b0 START / b1 STOP / b2 ACCUMULATE | CERTAIN |
| `+0x2F` | **psum_zero_region** | byte | `ceil(log2 bankspan)`, pow-2 ≤16 banks | CERTAIN |
| `+0x30` | **PSUM-DST descriptor** | `MEM_PATTERN3D` (16 B) | write, PSUM-resident; bank in `ADDR4` hi | CERTAIN |

The dst descriptor (`+0x30`) and the accumulate/zero-region/dtype bytes (dst-dtype `+0x23`-dense/`+0x28`-MX, `+0x2B`, `+0x2F`) are **siblings** in the bundle — together they fully describe the PSUM write: the descriptor the *address pattern* (incl. bank), the sibling bytes the *accumulate contract* (incl. dtype).

*Anchors: `core_v4::is_valid_matmul_regular` slot decode at `0x14b5cdb` / `0x14b5d9a` / `0x14b5db3`.*

> **GOTCHA — the matmul dst-dtype offset is per-generation, not a fixed `+0x28`.** The **dense** matmul (op `0x02`, CoreV2/V3) writes the dst dtype at `+0x23` and reads it back at `union+0x23`; there is no `+0x28` store in the dense encoder at all — that byte is pxor-zero. The **MX** matmul (op `0x100A`, CoreV4) writes and reads it at `+0x28`. The §5.3 decode is the CoreV4 validator, so its `union+0x28` reading is right for the MX struct only; generalizing it to the dense bundle reads a zero byte. Both offsets feed the same `==6` → bf16 `nc` path.

*Anchors: dense — `generateMatMul` `mov [r15+0x23],al` @ `0x1248882`, `core_v2::is_valid_matmul_regular` reading `[rsp+0xe3]`. MX — `generateMatmultMx` @ `0x143eded`, `core_v4::is_valid_matmul_regular` reading `[rsp+0x158]`.*
...
⇒ **3D is the workhorse DST form** (matmul PSUM dst, activation, TT, TS, quant); 2D is for the few 2-axis dsts; 4D for the 4-free-dim ops. **The matmul PSUM dst is always `MEM_PATTERN3D` at bundle `+0x30`.** The producer column is the enclosing function of each validator's call sites, per `nm -DC`.
...
- `mm_dst_mem3d_valid_nc_v4`: matmul PSUM-dst validator; addr **required** in PSUM window; dst dtype `{4..11}` (bf16 OR fp32); 3 nums `≠0`; reached when the dst dtype byte `== 6` (read at `union+0x28` by the CoreV4/MX validator; the dense CoreV2 validator reads `union+0x23` — §6).
