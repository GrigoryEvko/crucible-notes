...
sibling report at its stated confidence without re-reading the artifact. Every count is
grounded in `rg -c` / `nm` on the shipped binary, never the decompile.
...
> disassembled — interiors flagged INFERRED where they appear).

---

## 1. Opcode and ISA cluster
...
every gen — maintained and active SUNDA → MAVERICK with no deprecation.
...
confirms `PSEUDO_MEM_2D_STRUCT → {PSEUDO_TENSOR_LOAD, PSEUDO_TENSOR_STORE}` in all four gens.
...
## 2. What "LOAD" means here — source and destination memory spaces
...
runs `R0..R63`, `common.h:1009+`); a single `TensorLoad` names up to 32 of them.
...
> ↔ `0x100000` byte binding is a name-level match, not byte-traced. `[MED/OBSERVED]`
...
The partition-offset encoding of an SBUF vs PSUM address is documented in-header at
`common.h:455-468`: bit `[25] == 0` ⇒ SBUF, bit `[25] == 1` ⇒ PSUM; quadrant select in `[24:23]` (SBUF) /
`[21:20]` (PSUM).

---

## 3. The operand struct — `NEURON_ISA_TPB_MEM_2D_STRUCT` (64 B, compile-verified)
...
only the Immediate path, present for STORE and forbidden for LOAD.

---

## 4. dtype-extend on load — no float cast, integer sign/zero-extension only
...
element-wise numeric converters.

---

## 5. Dispatch surface — sequencer-native, not a POOL kernel
...
independently says `TensorLoad` = SEQUENCER engine. `[HIGH/OBSERVED — binary counts.]`
...
> `TensorLoad`: absent from the POOL table **and** 16-copy tag ⇒ **iTPB sequencer-native**.
...
It is neither a DVE compute op nor a Q7-POOL software kernel.
...
1. **(compile)** The compiler may emit `PSEUDO_TENSOR_LOAD (0xce)`, a `NEURON_ISA_TPB_PSEUDO_MEM_2D_STRUCT`
   ...
   *instead of* a resolved `start_addr`.
...
3. **(encode)** `instruction = MEM_2D_STRUCT`, `header.opcode = 0xaa`, `src_datasrc = 0`, `num_elem[0..1]`,
   `start_addr` (ADDR8), `step_elem[0..1]`, `data` = up to 32 destination register numbers.
4. **(validate)** `is_valid_tensor_load` (§3).
...
- **Alignment.** `start_addr` must be `dtype`-aligned (`is_aligned_start_addr8`, `mem_2d.h:160-162`). For
  `u64`+Register, every destination register number is even (`mem2d_u64_register_check`).
- **No multi-tile continuation.** There is no continuation inside one `TensorLoad` — it is bounded to ≤32
  ...
  (`DMA_MEMCPY 0xb8` / `DMA_INDIRECT 0xbb`).
...
## 8. Per-gen presence — byte-exact SUNDA → MAVERICK
...
| -------- | ------ | ------------------ | ------------------- | ------------------------ | ----------- | ----------------- |
| sunda    | NC-v2  | YES (`cmn:253`)    | YES                 | YES                      | YES         | PRESENT |
| cayman   | NC-v3  | YES (`cmn:250`)    | YES                 | YES                      | YES         | PRESENT |
| mariana  | NC-v4  | YES (`cmn:255`)    | YES                 | YES                      | YES         | PRESENT |
| maverick | NC-v5  | YES (`cmn:258`)    | YES                 | YES                      | YES         | PRESENT |

`struct2opcode.NEURON_ISA_TPB_MEM_2D_STRUCT` returns exactly `["…TENSOR_LOAD","…TENSOR_STORE"]` in **all four**
`instruction_mapping.json` files (verified with `jq`). `TENSOR_LOAD (0xaa)` is therefore ISA-defined,
maintained (`// Y`), and 64 B-struct-identical in every gen from the earliest (SUNDA/NC-v2) — no
gen-presence asymmetry, no deprecation.
...
> not disassembled; its presence is asserted from the header + the 16-copy `S:`-tag count, not
...
SEQ-blob residency: `S: TensorLoad` = 16, matching `WRITE`/`MOVE`/`TensorStore`. `[HIGH/OBSERVED;
...
not the compute-dtype set. See the full GPSIMD [datatype model](./dtype-model.md) for how
these enum bytes map across the whole ISA.

---

## 10. Relationship to TensorStore (`0xab`)
...
- [SBUF + PSUM bank model](../../dma/sbuf-psum-banks.md) — the on-chip State-Buffer / PSUM bank geometry;
  explains the partition-offset encoding `TensorLoad` addresses with.
...
- dispatch surface: **absent** from every POOL `kernel_info_table` (SUNDA/CAYMAN/MAVERICK carves) ⇒ not a POOL
  software kernel; 16-copy `S: TensorLoad` SEQ blob ⇒ iTPB sequencer-native (counts on the binary).
...
(0xbb)`, which *are* POOL kernels. Additionally (not in the `mem_2d.h`-only view): the
