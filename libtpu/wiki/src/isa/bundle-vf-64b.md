...
Viperfish (TPU v5e) is the first TPU TensorCore generation whose VLIW issue word is **64 bytes** (512 bits) wide, up from Pufferfish's 51 bytes and Jellyfish's 41. The width is not derived by zero-extending a narrower bundle — it is returned as an inline constant from `ViperfishCodecMetadata::BundleSizeBytes` (@ `0x1ee71320`, returns `64`), one cell of the `(TpuVersion, TpuSequencerType)`-keyed codec-metadata table described on the [Bundle Model](bundle-model-overview.md) page. The 64-byte bundle is shared, byte-for-byte at the slot level, by the two later V5-family generations Ghostlite (v5p) and Trillium (v6e); this page documents Viperfish specifically and notes where the family diverges.

The structural break from the JF/PF model is the encode mechanism, not just the width. Jellyfish and Pufferfish each have a single monolithic encoder — `EncoderJf::EncodeBundleInternal` (@ `0x1e86c7c0`) and `EncoderPfTensorCore::EncodeBundleInternal` (@ `0x1e8c5c40`) — that walks one `Bundle` object and packs every slot inline. Viperfish has **no `EncodeBundleInternal`**. Instead, a generic `EncodeBundle` orchestrator (@ `0x1e838cc0`) dispatches on `(TpuSequencerType)` to a `TensorCoreCodecBase<…>` whose template argument list *is* the slot-encoder walk order, and each slot is serialized by its own `<Slot>Encoder::Encode` method. Those per-slot encoders are populated upstream by `xla::tpu::sparse_core::isa_emitter::EmitX` proto-template functions (`EmitImmediate`, `EmitBranchOp`, `EmitCallOp`, `EmitPredicationToSlot`) rather than by per-slot `Encode` methods on the `Bundle`. Every field write — opcode, operand, predicate — goes through one universal bit-granular packer, `BitCopy(dst, dst_bit, src, src_bit, nbits)` (@ `0x1fa0a900`, LSB-first), so the whole bundle layout is expressed as a flat list of *(absolute-bit, width)* triples rather than a struct cast.
...
- The V5+ two-stage encode chain: `isa_emitter::EmitX` populates a proto submessage; `<Slot>Encoder::Encode` then `BitCopy`s each proto field to a generation-fixed *absolute bit* in the 512-bit buffer. There is no `EncodeBundleInternal`.
...
| | |
|---|---|
| **Generation** | Viperfish — TPU v5e (`TpuVersion` enum, per [Bundle Model](bundle-model-overview.md)) |
| **Namespace** | `asic_sw::deepsea::vxc::isa` (TC), `asic_sw::deepsea::vxc::vfc::isa` (SparseCore TEC/SCS) |
| **Bundle width** | **64 B / 512 bit** — `ViperfishCodecMetadata::BundleSizeBytes` @ `0x1ee71320` returns `64` |
| **Width dispatch** | `codec_metadata::BundleSizeBytes(TpuVersion, TpuSequencerType)` @ `0x1ecf7180` → `GetMetadataOrDie` → vtable +16 |
| **Encode entry** | `EncodeBundle` @ `0x1e838cc0` → `TensorCoreCodecBase<…>` worker @ `0x1d371540` (per-slot `Encoder::Encode` walk) |
| **Universal packer** | `BitCopy(dst, dst_bit, src, src_bit, nbits)` @ `0x1fa0a900` — bit-granular, LSB-first (`byte = dst_bit>>3`, `bit = dst_bit&7`) |
| **MXU slots** | `TensorCoreVectorExtended0Encoder::Encode` @ `0x1efa0f60` (bound `0x66`=103 ops), `…Extended1Encoder::Encode` @ `0x1efec020` |
...
| **MXU op census** | 94 `VectorExtended0` + 94 `VectorExtended1` per-op helpers; 0 `Extended2/3` (exactly two MXU issue slots) |
...
```text
EncodeBundle @0x1e838cc0                       ── codec dispatch on TpuSequencerType
  ├─ (TC)  → TensorCoreCodecBase<…> ctor @0x1d371e80
  │           worker @0x1d371540               ── walks each slot Encoder in template-arg order
  ...
  │             └─ TensorCoreVectorLoad{0,1}Encoder::Encode
  ├─ (SCS) → SparseCoreScsCodecBase<…> ctor @0x1391cd40
  └─ (TEC) → SparseCoreTecCodecBase<…> ctor @0x1391d4c0
```
...
```c
function EncodeBundle(out_buf, bundle, sequencer_type):   // sub_1E838CC0 @0x1e838cc0
    codec = SelectCodec(sequencer_type)        // TC / SCS / TEC — cmp 0 / 5 / 3
    worker = codec.bundle_worker               // TC → 0x1d371540
    // template-arg order IS the slot walk order; each slot is one Encoder::Encode
    for each SlotEncoder in codec.slots:
        SlotEncoder.Encode(bundle.<slot_proto>, out_buf)   // BitCopy into out_buf

function TensorCoreVectorExtended0Encoder::Encode(proto, out):   // @0x1efa0f60
    BitCopy(out, 64, &proto[0x1c], 0, 4)       // MXU-id (unit) written FIRST, always
    switch (proto[0x50]):                       // proto opcode — the op-family dispatch
        case 12: EncodeMatrixMultiplyBf16(out, proto)   // proto opcode 0xc
        case 65: EncodePushmatrixBf16(out, proto)       // proto opcode 0x41
        case …:  // 94 op families total, bound 0x66 (103) checked by the dispatcher
```
...
|---|---|---|---|---|
| result-type discriminator | 24 | 4 | selects PopMxu / PopEup / Transpose / PopCcrf | CERTAIN |
| result mode/format | 22 | 2 | per result-type | CERTAIN |
| dest vreg | 14 | 6 | common tail (proto + 0x18) | CERTAIN |

The four Viperfish sub-messages (proto types under `asic_sw::deepsea::vxc::isa`):

| Sub-message | Result opcode | Role | Confidence |
|---|---|---|---|
| `TensorCoreVectorResult_PopMxuResult` | 6 | drain a finished MXU matmul accumulator | CERTAIN |
| `TensorCoreVectorResult_PopEupResult` | 7 | drain an EUP/transcendental result | CERTAIN |
| `TensorCoreVectorResult_TransposeResult` | 5 | drain a systolic transpose | HIGH |
| `TensorCoreVectorResult_PopCcrfResult` | — | scalar/cross-core-register-file pop (vxc-only) | HIGH |

> **QUIRK —** `PopCcrfResult` is unique to Viperfish. Ghostlite drops it and adds `PopAddMxu01Result` (a fused matres+accumulate for the K>128 multi-pass path, where the accumulate is folded into the result pop rather than issued as a separate VALU add). Trillium keeps only `PopMxu`/`PopEup`/`Transpose`. A cross-generation reimplementation must key the result sub-message set on the generation, not assume it is constant.
...
Viperfish uses a **per-slot** 4+1 predicate field (4-bit reg index + 1-bit inversion, with a 2-bit rotating extension) at the top of each scalar slot — for `TensorCoreScalarAlu0` the reg is @ bit 499 (w4) and inversion @ bit 503 (w1). This is the encoding the later Trillium generation replaces with a 2-bit dual-selector indexing a dedicated `TensorCorePredicates` pool; on Viperfish the predicate index is local to each populated slot. Empty slots carry the `kNeverExecute` predicate written into the bundle header (see [Bundle Model](bundle-model-overview.md)).
...
|---|---|---|---|
| Encode entry | one `Encoder<gen>::EncodeBundleInternal` packing all slots | `EncodeBundle` orchestrator → per-slot `<Slot>Encoder::Encode` | CERTAIN |
...
The clean way to state the break: Pufferfish's `EncodeBundleInternal` *is* the kIsaTable for its slots, baked into one function. Viperfish has no such function — its kIsaTable is the *set* of per-slot `BitCopy` offsets, distributed across ~200 per-op helpers, orchestrated by `EncodeBundle`. The InstBits table that LLVM-MC would normally hold the fixed instruction bits in is entirely zero on disk for all V5+ generations, which is the binary's confirmation that the bits come from the emitter path, not from a static table.
...
| Ghostlite / Trillium | The other two 64-byte V5+ generations; same slot grid, byte-shifted opcode/predicate regions, differing result sub-message sets |
