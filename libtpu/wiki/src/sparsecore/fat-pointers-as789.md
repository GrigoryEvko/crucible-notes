...
> **CORRECTION (P-3-352) —** earlier analysis left the AS7/8/9 reserve labelled an *inferred TPU structured-pointer* — a `{core/chip, space, offset}` packing the SparseCore was hypothesised to use for cross-core addressing. Instruction-grade re-analysis overturns that: the reserve is dead AMDGPU boilerplate, there is no constructor, and the cross-core/cross-tile routing rides separate SSA operands (destination-id for remote, `tpu_tileid` for on-tile), not pointer bits. The "structured-pointer reserve" claim is now a CONFIRMED dead-reserve.
