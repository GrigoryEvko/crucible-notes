...
The death is exhaustive. Across all three wheels, the only textual occurrences of `blockwise_mm_shard_intermediate_mx` / `..._mx_hybrid` are (a) their own two `def` lines in `bwmm_shard_on_I_mx.py`, and (b) `moe_cte.py:337` / `:339`, which sit **inside the docstring Pseudocode block** (`:325-340`), not in code. There is zero real import, zero real call; no test, no `_private_kernels` `.so` source-reference, and (since NxD is a separate pip package not bundled inside the neuronx-cc wheel) no in-artifact caller. The two MX-I variants are reachable **only** by a direct deep import of the leaf function. (Provenance: D-O01 §0, D-O05 §5.)

> **CORRECTION (NX-065 / S2-09) —** earlier notes presented enums 5/6 in parentheses as if wired (`shard_on_i_mx = 5 -> (blockwise_mm_shard_intermediate_mx)`) and listed all seven leaves as reachable "HIGH" dispatch targets. The leaves *exist* and the descriptor is right for leaf existence, but enums 5/6 are **unreachable through `moe_cte()`**. The parenthesised entries are aspirational/pseudocode-only.
...
> **CORRECTION (D-O02 §H2) —** the `bwmm_shard_on_block` docstring (`:15`, `:101-102`) promises a "static + dynamic / hybrid early-exit" loop, but the shipped body sets `use_dynamic_while=False` and `NUM_STATIC_BLOCKS=N` and contains only a `# STATIC LOOP` — no dynamic while, no early-exit, no `conditions` arg in this leaf's signature. The hybrid/dynamic path lives only in the `_I` and `_mx` siblings. (`n_block_per_iter`, forwarded by the dispatch, is also accepted but never read in the body — vestigial. And `down_activations` is reset to `None` mid-body, so block-shard activation-checkpointing is effectively a no-op in this leaf.)
...
> **CORRECTION (D-O01 §3 framing) —** a grep of the dropping leaf for `capacity|drop|overflow|mask|discard` finds **zero** capacity-factor, overflow test, or drop-mask. The token-DROPPING (capacity-overflow discard) happens **upstream** in the router that builds `token_position_to_id` / `expert_affinities_masked`; the kernel only consumes the already-capacity-bounded layout. What "dropping" *is* in-kernel: the layout where `N == E` (one block per expert), so `block_idx == expert_idx` — the kernel does not even read `block_to_expert`, it memsets `block_expert = block_idx` (`:1906-1907`). Each block is a full per-expert capacity (`B >= 1024`), so it B-tiles the block into `MAX_BLOCK_TILE_SIZE=1024` chunks. Activation-checkpointing is always on (training layer). "Dropping" names a *layout*, not a runtime drop.
...
> **CORRECTION (cross-family) —** do not extrapolate block-shard's static-only finding to the whole MoE-CTE family. The hybrids in `bwmm_shard_on_I` (enum 2) and `bwmm_shard_on_I_mx` (enum 6) are **real** dynamic-while loops; only `bwmm_shard_on_block`'s docstring overstates.
