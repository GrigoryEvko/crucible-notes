...
- **The throughput integer is `array[15]` (matmul) or `array[0]` (matpush) of the matched [MXU-latency](mxu-latency-vf.md) reservation row.** The *dispatch* (`res 3 → array[15]`, `res 0xb → array[0]`) is byte-confirmed, and the three throughput cells are byte-anchored to the ctor (matmul `array[15]={8,16,32}`, matpush `array[0]=4`); the full per-`MatmulModifier` matrix is owned by the [MXU-latency](mxu-latency-vf.md) / [Performance: VF](performance-vf.md) pages (see NOTE (VF-CT-1) below).
