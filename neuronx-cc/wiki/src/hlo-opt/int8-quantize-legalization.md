...
- [4.29 — ScaledMatmul / MX-FP8 device path](scaled-matmul-mx.md) — the real device quantized GEMM (Path D), the U32-packed microscaling consumer
- [4.30 — Calibration and scale/zero-point flow](calibration.md) — `CalibratedQuantizedType`, the oneDNN INT8 matmul scale/zp flow (Path C in depth)
...
- [bir::CastToNewDType](../bir/cast-to-new-dtype.md) — the low-level numeric int cast that quantize operands ultimately ride on the device
