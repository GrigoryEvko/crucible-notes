...
| **3. Megascale / barrier-assignment config** | `BarrierConfig` proto with `barrier_type()`; per-core sflag/DMA barrier | `InferBarrierConfig` `0x1376c240`; `BarrierAssignment` `0x109c6e40`; `TensorCoreBarrierAssignment` `0x109c7060`; `SparseCoreBarrierAssignment::AssignBarriersForKernels` `0x109c6080` | Yes — runtime sync (sflag/DMA) |
