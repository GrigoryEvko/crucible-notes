...
> **Stock LLVM 20.0.0 with NVPTX target hooks.** No CUDA-specific modifications to the outlining algorithm were detected — the candidate finder, suffix-tree match algorithm, benefit threshold knob, and `nooutline` attribute check all match upstream byte-for-byte. The NVPTX delta is limited to two target hooks: `NVPTXInstrInfo::getOutliningType` (which decides per-MI whether an instruction is outlinable) and the calling convention assignment (CC 95) used for outlined stubs. The activation policy comes from `NVPTXPassConfig::addMachineOutliner`, not from the outliner itself.
>
