...
`__init__` (`0xe9a0`, `:41`) is a flat four-attribute store sequence — a direct `tp_setattro` chain over `:42..:46`:
...
`tensor_map` is the **memo of already-cast weights**: a const tensor cast to a given dtype is built once and shared across all its loads (see `staticCastLoadTensor`). `set_function(f)` (`0xc9d0`, `:51`) re-points the pass at a new IR with a single `setattr "f"` — no state re-allocation. `reset()` (`0xd2f0`, `:48`) is `self.tensor_map.clear()` — getattr `tensor_map`, getattr `clear`, call — discarding the cast memo between runs.

Two attributes a reader might expect here are conspicuously absent. Neither `allowed_dtypes` nor `enable_dma_cast` is set in `__init__`; both are read elsewhere, off different objects. `allowed_dtypes` is read off the **operand** (`matchOrMutateDstTy_0x10ed0.c:1184`), and `enable_dma_cast` is reached through a `_GetModuleGlobalName`/getattr chain inside **`mutateLoad`** (`mutateLoad_0x185f0.c:1280`) — that is, off `self.target`, not `self`. The attribute names and call sites are exact; Cython temp aliasing means the receiver attribution is reasoned rather than read.

## `MutateState` — the three-valued result

`MutateState` is a small enum-like result type with three members. The per-member accessor methods are compiled at `MutateState_1mismatch` @ `0x10150`, `MutateState_5mutated` @ `0x10810`, `MutateState_3match_or_mutated` @ `0xfa90`, all three present in `_function_addresses.json`.
...
The lowercase string members `mismatch` / `mutated` / `match_or_mutated` are the compiled accessor names; `tryMutateDstTy` reads `result.mismatch` directly. `match_or_mutated` is the convenience predicate `state in {Match, Mutated}` — "the dtype is satisfiable here." The member objects and accessor addresses are read from the binary; that reading of the predicate's meaning is INFERRED from caller usage.

`tryMutateDstTy` (`0xdb20`, `:162`) is the thin boolean adapter: it calls the core predicate with **keyword** args and negates the mismatch flag:
...
This is the deliverable. `matchOrMutateDstTy` (`0x10ed0`, `:81`) walks an ordered conjunction of gates; it returns `Mismatch` at the *first* failing gate, `Match` on the early dtype-equality exit, and only performs the mutation + returns `Mutated` if **all** gates pass. The ordering below is the order the `getattr`/`isinstance`/`RichCompare` operations execute in the decompiled body. Every attribute and method **name** below is read directly. The source-line numbers are firmer in some places than others: Cython emits a *literal* `_Pyx_AddTraceback` marker at `:81`, `:82`, `:85`, `:86`, `:89`, and `:117`, while `:95`, `:101`, `:104`, `:107`, `:110`, `:113`, `:120`, `:122`, and `:124` carry their line in a runtime variable and are reconstructed. Two reads (`operand` against `self`) have their receiver reasoned rather than read, because Cython temp aliasing (`v11`/`v37`/`obj`) obscures it.
...
The pass registers its four counters through `register_stats` imported from `neuronxcc.starfish.penguin.Statistics` — the `_Pyx_ImportFrom "register_stats"` sits in the module-exec init, and both strings are in the pool.

## Evidence summary

Read directly from the cp310 module artifacts:

- All 11 method and accessor addresses are in `_function_addresses.json`: `matchOrMutateDstTy 0x10ed0`, `mutateLoad 0x185f0`, `staticCastLoadTensor 0x13c90/0x17b10`, `tryMutateDstTy 0xdb20`, `__init__ 0xe9a0`, `reset 0xd2f0`, `set_function 0xc9d0`, and `MutateState.mismatch/mutated/match_or_mutated 0x10150/0x10810/0xfa90`.
- `canMutateDstTy` appears as a single `.rodata` string-pool constant at `0x1c6b0` (`_full.c:1036`, referenced from code at `0x5711`), consumed by exactly one `getattr` (`matchOrMutateDstTy_0x10ed0.c:1649` = `_full.c:11398`), and is absent from the function table — hence a property, not a function.
- The module docstring `"DTypeMutator - Mutate dtype for TongaISAInst."` is a substring of the full Copyright docstring in the pool; all four dtype literals and all four Statistics description strings are in `_strings.json`.
- Every gate's `getattr` / `isinstance` / `RichCompare` and every dtype literal sits at the decompile line cited: `enable_dma_cast` at `mutateLoad_0x185f0.c:1280`; the `"Can only cast weight"` guard and `is_const` at `staticCastLoadTensor_0x13c90.c:561`; the `tensor_map` memo subscript at `:970`/`:2694`; `dt.static_cast` at `:2413`.

### Limits of this reading

Most per-statement Python source-line numbers (`:95`, `:101`, `:104`–`:160`, `:56`, and others) are reconstructed rather than read. Cython emits a *literal* line in `_Pyx_AddTraceback` at only a subset of sites; in this module those literals are `41, 48, 51, 162, 54, 67, 81, 82, 85, 86, 89, 117, 127`. Every other line number travels through a runtime `vNN` variable. The line *ordering* and the *attribute name* at each step are not in doubt.

The exact *receiver* of a few attribute reads — `allowed_dtypes` off `operand` against `self`, `mutate_multi_users` off `self` — is reasoned, not read, because Cython temp aliasing (`v11`/`v37`/`obj`) blurs the object. The attribute names and their source lines are pinned by `AddTraceback`; only the disambiguation is inference.

The `"_8"` infix in the clone-naming (`:67`–`:69`) and its `n_loads`-dependent arithmetic are the weakest claim on this page: the `kp` constant was not byte-recovered from the dumped pool, and Cython's unicode-width temps obscure the precise string build. The structure — name plus marker plus `allocateId` — is solid; the literal suffix is not.

Finally, the `Match` / `Mutated` / `Mismatch` *semantics* relative to a propagation driver, and the pipeline ordering against AutoCastFP32 and cast-elimination, are **INFERRED** from caller usage and sibling passes rather than from this module's body.
