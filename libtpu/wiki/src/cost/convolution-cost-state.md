...
| Offset | Content | Store site / source | Confidence |
|---|---|---|---|
| `+0x0` | `const HloInstruction* conv` | `*(_QWORD*)a3 = ExtractConvLikeHlo(inst)` `@0x130a4b4f` | CERTAIN |
| `+0x8` | `inlined_vector<long,6>` — **operand-shape** chunk counts | `ChunkCountsWithTmp(operand_shape)` → `Assign(a3+1)` `@0x130a4c25` | CERTAIN |
| `+0x78` | `inlined_vector<long,6>` — **output-shape** chunk counts | `ChunkCountsWithTmp(output_shape)` → `Assign(a3+15)` `@0x130a4c38` | CERTAIN |
| `+0xe8` | `inlined_vector<long,6>` — **kernel-shape** chunk counts | `ChunkCountsWithTmp(kernel_shape)` → `Assign(a3+29)` `@0x130a4c58` | CERTAIN |
| `+0x158` | `ConvolutionDimensionNumbers` | `CopyFrom((char*)a3 + 344, …)` `@0x130a4b79` | CERTAIN |
| `+0x1e8` | `Shape` (operand/kernel shape) | `Shape::operator=((char*)a3 + 488, …)` `@0x130a4b90` | CERTAIN |
| `+0x328` | `Window` | `Window::CopyFrom((char*)a3 + 808, …)` `@0x130a4baa` | CERTAIN |
| `+0x350` | output-feature dim SIZE (physical) | logical→physical `[+106]` `@0x130a51f7` | HIGH |
| `+0x358` | input-feature dim SIZE (physical) | logical→physical `[+107]` `@0x130a5212` | HIGH |
| `+0x360` | output-spatial dim SIZE (physical) | logical→physical `[+108]` `@0x130a5235` | HIGH |
| `+0x368` | input-spatial dim SIZE (physical) | logical→physical `[+109]` `@0x130a5250` | HIGH |
| `+0x370` | kernel-feature dim SIZE (physical) | logical→physical `[+110]` `@0x130a5279` | HIGH |
| `+0x378` | kernel-spatial dim SIZE (physical) | logical→physical `[+111]` `@0x130a529b` | HIGH |
| `+0x380` | `vector<bool>` — per-dim mask A (batch-dim) | `assign(N, 0)` `@0x130a52bb` | CERTAIN |
| `+0x398` | `vector<bool>` — per-dim mask B (outer-dim) | `assign(N, 0)` `@0x130a52dd` | CERTAIN |
| `+0x3b0` | `vector<bool>` — per-dim mask C (inner-dim) | `assign(N, 0)` `@0x130a52ff` | CERTAIN |
| `+0x3c8` / `+0x3e0` / `+0x3f8` | `vector<long>` (init = 1) | `assign(N, 1)` `@0x130a531e/533d/535c` | CERTAIN |
| `+0x410` | `vector<bool>` | `assign(N, 0)` `@0x130a537e` | CERTAIN |
| `+0x428` | `vector<long>` (init = 1) | `assign(N, 1)` `@0x130a539d` | CERTAIN |
...
| CCS offset | Decimal | Flag (inferred name) | Read in `RecordConvKernelCycles` | Confidence |
|---|---|---|---|---|
| `+0x470` | 1136 | `perform_batch_packed_activation_reorg` | `== 1` gate `@0x130cb040` | HIGH |
| `+0x476` | 1142 | `perform_activation_if_bf16_packing` | `!= 0` / `== 1` gate `@0x130cb1b8/cb2bb` | HIGH |
| `+0x477` | 1143 | `generate_combined_x8_packed_matmuls_and_latches` | OR-combine `@0x130caf38` | HIGH |
| `+0x478` | 1144 | `generate_combined_x4_packed_matmuls_and_latches` | OR-combine `@0x130caf38` | HIGH |
| `+0x479` | 1145 | `generate_post_xpose_x8_packed_matmuls` | OR-combine `@0x130caf38` | HIGH |
| `+0x47a` | 1146 | `generate_bf16_packed_vmatmuls` | `^1` index-select `@0x130cb37e` | HIGH |
| `+0x47e` | 1150 | `perform_activation_bf16_packing` | OR-combine `@0x130caf38` | HIGH |
| `+0x480` | 1152 | `generate_x8_packed_vlatches` | `== 1` (0.25 quarter-rate gate) `@0x130cb468` | HIGH |
| `+0x481` | 1153 | `generate_x8_packed_vlatches_for_activations` | OR-combine `@0x130caf38` | HIGH |
| `+0x482` | 1154 | `generate_x8_packed_vmatmuls` | OR-combine `@0x130caf38` | HIGH |
...
| Offset | `a3[k]` | Content (inferred role) | Confidence |
|---|---|---|---|
| `+0x0` | `a3[0]` byte | shape-case flag ("input == kernel" class) | HIGH |
| `+0x2` | `a3[2]` byte | shape-case flag ("dim ≥ 2" class) | HIGH |
| `+0x3` | `a3[3]` byte | feature-dim selector (used as `[+0x3]^1` index pick) | HIGH |
| `+0x5` | `a3[5]` byte | `HasNonBatchDotBaseDilation` result `@0x130b79fc` | CERTAIN |
| `+0x6` | `a3[6]` byte | grouped-conv gate (`== 1`) | HIGH |
| `+0x8` | `a3[1]` | spatial / output dim product | HIGH |
| `+0x10` | `a3[2]` | spatial / output dim product (minor gate) | HIGH |
| `+0x18` | `a3[3]` | spatial dim product | HIGH |
| `+0x20` | `a3[4]` | output dim product | HIGH |
| `+0x40` | `a3[8]` | feature-group / kernel-feature product (read in `ShouldPackInputFeature`) | HIGH |
| `+0x48` | `a3[9]` | window-volume / spatial product | HIGH |
| `+0x50` | `a3[10]` | the M dim | HIGH |
| `+0x60` | `a3[12]` | feature dim product (`[+0x3]^1`-indexed pair with `+0x68`) | HIGH |
| `+0x68` | `a3[13]` | feature dim product (sibling of `+0x60`) | HIGH |
| `+0x70` | `a3[14]` | output-spatial iteration count | HIGH |
| `+0x78` | `a3[15]` | window-iteration count | HIGH |
...
> **NOTE —** the *semantic* role of each `ConvState` dim offset (which is output-feature vs input-feature vs spatial vs batch vs kernel-window) is reasoned from the `imul` chain and the `ChunkCountsWithTmp` source order, not from a debug layout — this build carries no DWARF. The offsets and the formula shape are byte-confirmed; the dimension *names* are HIGH-confidence inference. The authoritative source would be `GetConvIterationCounts` `@0x130b3c80`.
...
| CT | Call | Role | Confidence |
|---:|---|---|---|
| 0 | `MxuLat.GetResourceUsage(212 matmul', res 3, false)` | **the conv matmul-rate** `thru(CT 0)` | CERTAIN |
| 1 | `MxuLat.GetResourceUsage(218 matmul'', res 3, false)` | matmul-rate (alt) | CERTAIN |
| 4 | `MxuLat.GetResourceUsage(230 matmul, res 3, false)` | matmul-rate (base) | CERTAIN |
| 5 | `MxuLat.GetResourceUsage(267 matpush, res 11, false)` | matpush-rate | CERTAIN |
| 6 | `MxuLat.GetResourceUsage(271 matpush', res 11, false)` | matpush'-rate | CERTAIN |
| 9 | `MxuLat.GetResourceUsage(277 matpush'', res 11, false)` | matpush''-rate | CERTAIN |
| 10, 16 | `LogMessageFatal("Unsupported PushGainsS4.")` | abort | CERTAIN |
| 11 | `MxuLat.GetResourceUsage(267, res 11, **true**)` | matpush-rate (transposed) | CERTAIN |
| 12 | `MxuLat.GetResourceUsage(271, res 11, **true**)` | matpush'-rate (transposed) | CERTAIN |
| 15 | `MxuLat.GetResourceUsage(277, res 11, **true**)` | matpush''-rate (transposed) | CERTAIN |
| 23 | `VfPerf.GetResourceUsage(296, res 14) + 1` | mxres / Xlu (matres TC) | CERTAIN |
| 27 | `VfPerf.GetResourceUsage(302, res 14) + 1` | mxres | CERTAIN |
| 28 | `VfPerf.GetResourceUsage(287, res 14) + 1` | **the conv MXRES (Xlu)** | CERTAIN |
| 29 | `VfPerf.GetResourceUsage(291, res 14) + 1` | mxres | CERTAIN |
| 31 | `VfPerf.GetResourceUsage(298, res 14) + 1` | mxres | CERTAIN |
| all others | `return 1` | vector / matprep-bucket default | CERTAIN |
...
| `thru(CT 0)` — matmul rate (`array[15]` = `MatmulAccA`, per `MatmulModifier` format) | value | Confidence |
|---|---:|---|
| bf16 (format group 1) | 8 | CERTAIN |
| format group 2 | 16 | HIGH |
| int8 / wide (format group 4) | 32 | HIGH |

| `thru(CT 5)` — matpush rate (`array[0]` = `MatpushPushPort`, per `MatpushModifier`) | value | Confidence |
|---|---:|---|
| bf16 narrow | 2 | CERTAIN |
| bf16 mid | 4 | HIGH |
| int8 / x8 wide | 8 | HIGH |

> **NOTE —** the bf16 `array[15] = 8` and `array[0] = 2` are end-to-end confirmed; the non-bf16 entries are inferred from the per-format reservation groups and would be pinned by dumping the live `MatmulModifier`/`MatpushModifier` maps per `MatmulDataFormat`. The `{2,1,1}` / `{4,3,2}` / `{8,7,6}` reservation triplet's **first** element (the `MatpushPushPort` value) is exactly `thru(CT matpush)`; the other two (matrix-staging-register holds) are read by separate `GetResourceUsage` calls in the MXU scheduler, not by this bridge.
...
| Slot | `Acc` index | Deposit | Site | Confidence |
|---|---:|---|---|---|
| `Matmul` | `R[1]` | `op_count × thru(CT 1) × 0.5 ÷ Target+0x4ac ÷ ElementPackingFactor` | `@0x130cb...` (Acc `@line 560`) | CERTAIN |
| `Matpush` | `R[0]` | `matpush_count × thru(CT matpush)` | Acc `@0x130cb...` (`@line 609`) | CERTAIN |
| `Xlu` | `R[2]` | `thru(CT mxres) × ChunksPerTile × remainder ÷ Target+0x4b0` | Acc `@line 259/428` | CERTAIN |
...
|---|---|---|---|
| `CostModel::GetConvolutionCostState` | `0x130a4b20` | builds `ConvCostState` (3520 B) | CERTAIN |
| `CostModel::RecordConvolutionCycles` (4-RV) | `0x130b6ce0` | builds `ConvState`, calls the kernel leaf | CERTAIN |
| `CostModel::RecordConvKernelCycles` | `0x130caf20` | dim → MXU deposit (`R[0]`/`R[1]`/`R[2]`) | CERTAIN |
| `VfCycleTable::GetCyclesForThroughput` | `0x1c89e2c0` | CT → MXU-occupancy adapter | CERTAIN |
| `viperfish::MxuLatencyTable::GetResourceUsage` | `0x1c8ae5c0` | `res→index` remap + `array[idx]` | CERTAIN |
| `GlcCycleTable::GetCyclesForThroughputHelper` | `0x1c89ed20` | Ghostlite bridge (`array<int,11>`) | CERTAIN |
| `GfcCycleTable::GetCyclesForThroughputHelper` | `0x1c89f400` | TPU7x bridge (`array<int,11>`) | HIGH |
| `fusion_util::ExtractConvLikeHlo` | `0x1d6aa140` | conv-like HLO extractor | CERTAIN |
| `convolution_util::GetConvolutionLoweringStrategy` | `0x13192820` | the 21-byte flag block | CERTAIN |
| `Target::ChunkCountsWithTmp` | `0x10c8b6e0` | per-axis chunk grid → `+0x8`/`+0x78`/`+0xe8` | CERTAIN |
| `TransferSizeUtil::ElementPackingFactor` | `0x1d6b03e0` | per-dtype packing divisor | CERTAIN |
| `ResourceVector::Acc` | `0x1c89adc0` | the slot deposit | CERTAIN |
