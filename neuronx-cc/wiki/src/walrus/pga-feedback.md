...
The metric is the VN-Splitter knobs the search tunes; the [`vnsplitter-shrink`](vnsplitter-shrink.md) page documents the pass itself (pass 24, `vn_splitter`, the fold-then-split body PGA drives). PGA is a **grid search**, and it should not be confused with the separate penguin-frontend MCTS autotuner (the `penguin-autotuner` page, planned) which searches a tile/schedule space with Monte-Carlo tree search — a fundamentally different algorithm. The two share the word "autotuner" and nothing else.
...
> **QUIRK — split-then-fuse, not fold-then-split.** The standalone `VNSplitterPass::run` (`@0xd73890`, documented on [`vnsplitter-shrink`](vnsplitter-shrink.md)) runs **fold first** (`VerticalFusion`) then **split** (`VNSplitter`). The PGA candidate body runs them the **opposite** way: `VNSplitter::runTransform` (`d6fad2`) *before* `VerticalFusion::runTransform` (`d6febf`). A reimplementer must not assume the pass and the autotuner candidate share an order — PGA splits with the tuned knobs first, then re-fuses to measure the net footprint. `[CONFIRMED — call order directly disassembled]`
