...
The census is computed two ways, and the two disagree, which is the single most important caveat on this page. Counting by **leading typeinfo namespace** (the `_ZTIN<len><name>` prefix — "this class lives in namespace X") is the metric here: it answers "how many polymorphic classes does X define." Counting by **top token of the demangled name** (the metric an earlier pass used) over-counts header-only template libraries — `absl::StatusOr<xla::Foo>` and `Eigen::Matrix<...>` appear as the *return type* or *template argument* of thousands of typeinfos whose owning class is in some other namespace. Where the two diverge by an order of magnitude (`absl`, `Eigen`, `xla`), a `> **CORRECTION —**` note records both numbers and explains the gap.
...
| `absl` | 33 | ~33 | `absl::Duration` & status internals (`0x215fd610`) — see CORRECTION | CERTAIN |
...
> **CORRECTION (NS-XLA) —** an earlier pass reported `xla` at **5,291** typeinfos. That figure counted the top token of each *demangled* name, which credits `xla` for every `absl::StatusOr<xla::…>` and `std::unique_ptr<xla::…>` wrapper whose owning class is in `absl`/`std`. Counting by **leading typeinfo namespace** (`_ZTIN3xla…`) gives **3,036** — the number of polymorphic classes `xla` actually *defines*. The 5,291 figure is the "xla appears anywhere as the first token" count and is retained only as the upper bound.
...
> **CORRECTION (NS-ABSL-EIGEN) —** an earlier pass reported `absl` at **509** typeinfos and `Eigen` at **406**. Both are artifacts of demangled-top-token counting. Abseil and Eigen are overwhelmingly **header-only template** libraries: `absl::StatusOr<T>`, `absl::flat_hash_map<K,V>`, `Eigen::Matrix<…>` instantiate as the *outer* type of thousands of typeinfos, but the *owning polymorphic class* is almost always elsewhere. Counting by leading typeinfo namespace gives `absl` **33** and `Eigen` **4** — the number of genuinely polymorphic classes each defines (e.g. `Eigen::ThreadPoolInterface`). The 509 / 406 figures measure template-wrapper prevalence, a different and legitimate metric, but not class ownership.
