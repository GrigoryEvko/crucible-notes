...
> **CORRECTION (DCL-1) — the `PatternBenefit` is at `Pattern+0xc`, not `Pattern+0x8`.** The sort comparator (`__stable_sort_move` at `0x1c9614a0`) reads the tie-break key as `*(_WORD *)(*pair + 12)` — a `uint16` at `Pattern+0xc`. `Pattern+0x8` is the `RootKind` discriminator (a `uint32`), which the graph builder and `computeLegalizationGraphBenefit::$_0` test with `cmp [Pattern+0x8], 0x1` to decide "has a fixed root op." The `+0x8` that an earlier greedy-driver reading attributed to the benefit is the `SmallDenseMap<Pattern*,PatternBenefit>` *cost-cache* pair offset, not the `Pattern` member. The `impossibleToMatch` sentinel is the all-ones `uint16` `0xFFFF` (the `PatternBenefit(unsigned)` ctor stores the raw value with no clamp).
