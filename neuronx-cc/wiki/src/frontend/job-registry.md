...
A `neuronx_cc` compile is a sequence of **Jobs**. [The CompileCommand pipeline](compilecommand-pipeline.md) describes how `CompileCommand.buildPipeline` concatenates three collectors into an ordered list of job-*name* strings; this page describes the other half: how `JobRegistry` turns each name into a `Job` instance, and how each instance actually *runs* — either **in-process** (pure Python/Cython, no fork) or by **spawning a standalone tool ELF** out of `neuronxcc/starfish/bin/` and waiting on it.
...
> **GOTCHA — `Frontend` does not spawn `hlo-opt`, despite how the pipeline table reads.** The `Frontend` row of the pipeline table in [the CompileCommand pipeline](compilecommand-pipeline.md) is labelled `hlo-opt (--passes=…)`, which names the *conceptual* HLO-optimization stage, not a fork. `Frontend.so` carries only `runPenguin` / `runXLAFrontend` / `Penguin` (4 / 15 / 5 references), zero `Popen` sites and zero `getFullyQualifiedLocation` references. The `hlo-opt` ELF does ship in `starfish/bin/`, but no Job in this driver invokes it. `StaticIOTranspose` is the same story: it writes `io_transposes.json` with Python `json`, not through a native producer.
...
`Backend`'s tool name is a real literal, not a config-derived string: `Backend.cpython-310…so` carries `nn_executor` twice, as the interned identifier `__pyx_n_s_nn_executor` and as the raw constant `__pyx_k_nn_executor`, and the same basename reappears in `WalrusDriver.so`. Its argv shape — `ulimit -t <sec> && nn_executor --bir backend.bir --tensor_map <…> --sunda --json <…> …` — follows from the `backend.bir`, `--sunda`, and `ulimit` literals. The tool is concrete; it simply is not shipped in this wheel's `starfish/bin/`.

> **NOTE — three of the seven native tools are absent from this wheel.** `starfish/bin[/private]` ships only `walrus_driver`, `hlo2penguin`, `hlo-neff-wrapper`, `xla_infergoldens`, `hlo-opt`, `snapshot-unpack`, `coloring_allocator_with_loop`, `full_unroll`, `walrus_bugpoint_driver` (+ `private/tensor_indirect_test`). `nn_executor`, `kgraph2dot`, and `watchpoint_insert` are **not present**, so `Backend` (new-backend path) and `Watchpoint` (debug) would fail `getFullyQualifiedLocation` ("Could not find …") unless an internal build drops those bins in. They are experimental/debug Jobs, off by default. The four production tools' internals have their own pages.
...
The `private/` arm is searched **before** `starfish/bin`, and the ordering is fixed by the Cython string-pool layout. Cython lays out `__pyx_k_*` raw-string constants in **first-textual-use order**, and the data-section symbol addresses settle it: `__pyx_k_starfish_bin_private` resides at **`0x35d90`**, `__pyx_k_starfish_bin` at **`0x363f8`** — `private` is emitted *first*, so it is the first element of `pathDirectories` and the first segment of the `os.pathsep`-joined `which` path. The raw string bytes corroborate (`starfish/bin/private` at file offset `220560` < `starfish/bin` at `222200`). A `shutil.which` scans its `path` left-to-right and returns the first hit, so any tool present in *both* directories resolves out of `private/` — `private` **overrides** the shipped tool.

> **GOTCHA — `getAbsPath` is not the bin resolver.** The two path helpers are easy to swap. `getFullyQualifiedLocation` resolves *tools*, via a single `shutil.which` over one `os.pathsep`-joined string — `Job.so` references `shutil` / `which` / `pathsep` / `get_exec_path` 4 / 5 / 4 / 4 times, and there is no hand-rolled `os.path.exists` loop anywhere in it. `getAbsPath` normalizes *user-supplied command-line paths* against the launch directory and has nothing to do with tool lookup.
...
For the full per-Job argv templates (the ~200-flag `walrus_driver` corpus, the `hlo2penguin` flag set, etc.), see [sub-tool argv templates](subtool-argv.md).
...
The deep-bind flag is toggled **per job import**, not once around the enumeration loop. All of it lives in `__getJobFactory` (`0x10c00`), whose interned-symbol stream runs `getdlopenflags → setdlopenflags → RTLD_DEEPBIND → "neuronxcc.driver.jobs." → import_module → getfile → "Found"/"at" → "Could not import" → setdlopenflags` — three `setdlopenflags` sites in all: set-deepbind, restore-on-success, restore-on-exception. `loadAllJobs` references no RTLD or `setdlopenflags` symbol at all; it carries only `find_spec` / `get_filename` / `iter_modules` and the call into `__getJobFactory`.
...
Cross-references: [the CompileCommand pipeline](compilecommand-pipeline.md) (who builds the name list) and [sub-tool argv templates](subtool-argv.md) (the full per-Job flag corpus). The four production tool ELFs have their own pages.
