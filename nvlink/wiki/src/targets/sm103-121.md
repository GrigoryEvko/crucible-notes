...
The ordering is notable: sm_103 is registered **after** sm_110, not after sm_100 as the numbering might suggest. This likely reflects the chronological order in which these targets were added to the compiler toolchain -- sm_110 (Jetson Thor) was presumably defined before sm_103 (GB300 Blackwell Ultra).
...
The encoding table accessor functions (slot 3) return different parameter sets that control feature gating at the instruction level. While the exact parameter layouts have not been fully decoded, the pattern is consistent: each accessor populates a small structure (likely 8-32 bytes) that tells the encoder/decoder which instruction families and sub-opcodes are valid for the target architecture.
...
2. **Heterogeneous numbering** -- The five Blackwell architectures span three decades (10, 11, 12), creating three distinct compatibility groups despite sharing a single ISA. This suggests these architectures target substantially different silicon configurations (datacenter, automotive/embedded, consumer) even though the instruction set is common.

3. **Internal designations** -- The remapping table reveals three internal architecture numbers (101, 104, 130) that do not correspond to any user-visible `sm_` target. These are likely development or pre-release designations that were renumbered before public release.
