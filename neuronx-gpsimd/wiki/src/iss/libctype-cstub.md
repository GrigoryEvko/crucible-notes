...
> caller (the native sim) is trusted to pass `idx ∈ [0,63]`. `[OBSERVED · HIGH]`
...
ENFORCED** — a misaligned address is an unconditional hard-fault. `[OBSERVED ·
HIGH]`
...
the codec materialises the 1536-bit accumulator spill/reload host-side. `[OBSERVED
· HIGH]`
...
`idx63=wvecspill` — in exact agreement with the ctype/coproc ISA page. `[OBSERVED
· HIGH]`
...
* kind census `rtor205/loadi47/storei47` and the 9-zero kinds — re-grounded by
