...
| **ILP** | the `EnableIlpLatencyHidingScheduler` gate swaps the async *classifier* only — the comparator and `RunImpl` are unchanged; the named `ILPMemoryScheduler` MIP is dead code | [LHS ILP variant](lhs-ilp-variant.md) |

> **CORRECTION (SCHED-1) —** the flag family `xla_tpu_enable_ilp_latency_hiding_scheduler` suggests an integer-linear-programming scheduler replaces the greedy list scheduler. Static analysis shows otherwise: the genuine `xla::ILPMemoryScheduler` MIP exists but is unreachable (its vtable has zero code consumers and `GetMemorySchedulerAlgorithm` has no `ILP` enum case), and the live "ILP" path inside `RunHloScheduler` only swaps which async-op kinds are exposed as overlap candidates to the *same* greedy LHS. See the [ILP variant page](lhs-ilp-variant.md) for the two-path dissection.
