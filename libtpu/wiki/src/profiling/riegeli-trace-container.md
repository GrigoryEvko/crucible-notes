...
| **Factory key** | 12-byte `asic_sw::DeviceIdentifiers` PCI tuple (vendor `0x1AE0` = Google); 15 `kXxxChipIdentifiers` @ `.rodata 0xbdf3c0c`–`0xbdf3cdc` |
...
Fifteen `kXxxChipIdentifiers` PCI tuples are baked into `.rodata 0xbdf3c0c`–`0xbdf3cdc`; the `Is<Family>` predicates compare against them. All share `vendor=0x1AE0` and `subsystem_vendor=0x1AE0`. Rather than dump all fifteen rows, the discriminating axes are `device_id` (family) × `subsystem_device_id` (board) × `revision_id` (stepping):
