...
| **What the binary yields** | method rosters and field *names* come from the `__pyx_pw_` symbols and string pools; field *types and owners* do not (see § Compiled-Opaque Limits) |
...
> **NOTE — the bracketed `[5.N]` tags are a reading index, not the canonical Part-5 sequence.** Canonically, §5.4 is the [AffineExpr algebra over `pelican::Expr`](affine-expr-algebra.md) and §5.5 is the [dependency model](dependency-model.md); the three rows above are linked by slug to sidestep the collision.
...
`Value` (`ir/Value.py`) is the base SSA value. Its two distinctive interned identifiers are `name` and `kind`: `name` is the SSA name string (which, on the BIR side, becomes the dependency-edge hash key — BIR keys its inline dep-sets by the target's *name*, [Part 7](../bir/)), and `kind` is a `ValueKind` tag. `Value.py` imports `neuronxcc.pelican.ir`, marking it as the Python face of the C++ `pelican::Value`.
...
The module-level helper `wrap_compute_value` (in `ComputeValue.so`) boxes a raw `pelican::Value` into a Python `ComputeValue` — the boundary crossing from the C++ uniqued layer back to the Python IR.

> **QUIRK — RAUW carries an "Except" variant.** `replaceAllUsesWithExcept` (its own pyx symbol, `…ScalarValue_17replaceAllUsesWithExcept`) replaces all uses *but one* — the pattern a rewriter uses when it must keep its own reference live while redirecting every other consumer. LLVM has no built-in for this; Penguin needs it because the rewriters run in Python and frequently hold the old value while substituting.

### Instruction — the op-node base

`Instruction` (`ir/Instruction.py`) is the base of every op node. It is a `ComputeValue`+`User` subclass — it imports `neuronxcc.starfish.penguin.ir.ComputeValue`, `User`, `Value` and `Tensor`, plus the impl-binding back-edge `neuronxcc.starfish.penguin.targets.sunda.SundaISAInst`. Its roster is **exactly 51 distinct pyx methods**, counted from the `__pyx_pw_9neuronxcc_8starfish_7penguin_2ir_11Instruction_11Instruction_<n><method>` symbols. The field names `operands`, `results`, `attrs`, `_predicates`/`predicates`, `axes`, `loopnest`, `loopdepth` are all in the pool; that they belong to `Instruction` rather than a base is [INFERRED].

```c
class Instruction(ComputeValue, User):           // ir/Instruction.py, 51 pyx methods
    // fields (names only; types/offsets NOT recoverable — regular Python class)
...
Each of the seven predicate methods named above has its own indexed pyx symbol (`…Instruction_53is_predicated`, `_61addPredicate`, `_65projectPredicates`, `_67approxPredicates`, `_71canonicalizePredicates`, `_75evalMasks`, `_81enumerate_predicates_in_codegen_order`).
...
`Operator` (`ir/Operator.py`) ⊂ `Instruction` is the high-level, HLO-facing op layer — the graph the front-half printer ([Part 4](../hlo-opt/mhlo-to-python-printer-driver.md)) and `GradIRBuilder` build. The base adds `__init__`, `rhs_str`, `serialize`, `verify` and `verifyOperandType`, and subclasses add per-op operands/indices/axes (the whole §8 TensorOp family, owned by [the Operator family page](tensor-op-family.md)). Its distinctive module helpers are the **axis-role queries** the tiler and scheduler use to classify each `Axis` of an op (all present in `Operator.so`):
...
For `Tensor` values specifically, this use-list is overlaid with a **memory-access** view, because a tensor is read and written through `Access` descriptors rather than as a scalar SSA operand (methods from `Tensor.so`):
...
```c
class DependencyEdge:                             // ir/Dependency.py
    src   // an Instruction
    dst   // an Instruction
    kind  // Optional[EdgeKind]          (the type annotation is verbatim in the pool)
    __init__, serialize

enum EdgeKind { FLOW, ANTI, OUTPUT, ORDERED }    // members from the Dependency.so pool
    // FLOW = RAW (true producer→consumer); ANTI = WAR; OUTPUT = WAW;
    // ORDERED = scheduler/programmer-injected ordering
```

The edges are held by the `Function` — `Function.so` carries `add_dep_edge`, `dep_edges`, `dep_edges_for_inst`, plus `_dep_edges` as the backing field:
...
`EdgeKind`'s member set `{FLOW, ANTI, OUTPUT, ORDERED}` matches BIR's `bir::EdgeKind` `{Invalid0, Ordered1, Anti2, Output3, Flow4}` one-to-one (Penguin omits the `Invalid0` sentinel). The member names are read from the pool; the **numeric** values on the Penguin side are [INFERRED] from the BIR twin, not recovered directly.
...
`Module` (`ir/Module.py`, 14 methods + a `ModuleKind` enum) is the top container. Its docstring is verbatim in the pool: *"Modules contains a list of connected functions forming a neural network."* Its fields are `functions`, `subgraph_functions`, `root_function`, `kind`. The assert string *"expect exactly one function with attr 'main'"* fixes the invariant: a `Module` has exactly one main `Function`, and `subgraph_functions` are the callee sub-graphs.

### Function

`Function` (`ir/Function.py`, 53 methods) is the compilation unit — the object the `IRBuilder` ("cu") builds into. Its fields are `args`, `inputs`, `outputs`, `tensors` (with `ordered_all_tensors`/`findTensorByName`), `dep_edges` (Layer 2 above), `opt_level`, `target`. Nested classes: `BasicBlock`, `AliasType`. Its method groups cover blocks (`addBasicBlock`, `stmts_under_bb`), IO marking (`markInput`/`markOutput`/`ioTensors`), the alias model (`aliasTensors`/`findAliasTensor`/`buildAliasTensorMap`), the dependency graph (above), SPMD/sharding (`createShardId`/`replica_groups_table`), cost (`isComputeBound`/`isMemoryBound`), and serialization (`serialize_ir_string`/`verify`/`finalize`).

### BasicBlock and the Stmt layer

`BasicBlock` (nested in `Function.so`: `__init__`, `__str__`, `verify`) is the CFG node holding an ordered `Stmt` sequence — the Penguin analog of an MLIR `Block` / `bir::BasicBlock`. Inside a `BasicBlock`, the loop-nest/grouping wrappers live in `Stmt.so` as the nested classes `Block`, `Macro`, `DAGMacro` and `StmtGroup`:

```c
class Stmt:                                       // ir/Stmt.py
    Block      // the loop-body / tensor-scope statement; OWNS block-local tensors:
               //   addTensor, all_tensors, dropDeadTensors, stealChildren
...
A `Block` **owns** the tensors declared in its scope (block-local tensors) via `addTensor`/`all_tensors`/`dropDeadTensors`/`stealChildren`. `Macro`/`DAGMacro` are the schedulable fused units the tiler emits — they wrap a span of the §3-Inst roster into one node ([the Operator family page](tensor-op-family.md)).
...
class While:                                      // docstring, verbatim:
...
`While` is the structured data-dependent loop (guard + `continue_condition` + body), distinct from the affine-bounded `DynamicAxis` ([5.3](axis-loop-model.md)). `ScopeRegion` (`ir/ScopeRegion.py`, 7 methods) is a lighter named lexical region — a labeled scope-begin/scope-end pair bracketing a span of statements for analysis/codegen (e.g. a fusion-cluster or kernel boundary), with fields `parent`/`end`. `OptBarrier`/`CoreBarrierIntrinsic` are the no-reorder scheduling fences.

---

## The Round-Trip — Penguin IR as Runnable Python

A consequence of the open, constructor-based registry is that the IR's textual form **is** runnable Python. `IRWriter` (`ir/IRWriter.py`, 37 methods) is the textual emitter; it writes Python constructor calls and class assignments, and `roundtripIO` reads them back by executing the emitted code (`IRWriter.roundtripIO`, `IRWriter.roundtripIO.<locals>.read_ir_from_code`). Its `serialize_*` helpers are: `serialize_func_begin`, `serialize_basic_block_begin`, `serialize_dep_edge`, plus axis/dtype/block-end emitters.

> **NOTE — the printer and the in-memory IR share one textual form.** The Python-constructor surface `MhloToPythonPrinter` emits from the MLIR front half ([Part 4](../hlo-opt/mhlo-to-python-printer-driver.md)) is the **same** surface `IRWriter` writes for round-trip. The presence of `serialize_dep_edge` puts the `DependencyEdge` graph inside the serialized IR — the dependency edges survive a write/read cycle. Two serialize-format strings sit verbatim in the pools and pin the leaf nodes: a `Tensor` prints as `{attr}{dtype} {shape} {name}{init}` (`Tensor.so`), and an `AffineAxis` loop prints as `for ({it}: range({lb}, {ub}, {stride}))` (`Axis.so`) — a Penguin loop axis literally *is* a Python `for`-over-`range`.
...
- **Field types and offsets are not recoverable.** These IR classes are *regular* Python classes — Cython emitted no C-level `__pyx_obj_…<Class>` instance struct and no `tp_members`/`tp_getsets` for them, so their fields live in `__dict__`. The string pool gives field **names** but not their static types or byte-offsets. A field's **owning class** is settled only where a method roster, docstring, or serialize-format pins it; cross-cutting names like `shape`/`axes` are attributed by nearest method context and are correspondingly weaker in the per-family pages.
- **`EdgeKind` integer values** are [INFERRED] from the BIR twin; only the member names are recovered on the Penguin side.
