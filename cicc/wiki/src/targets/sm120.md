...
The upstream LLVM 22 NVPTX backend (`NVPTXSubtarget.h`) independently confirms this: `hasTcgen05InstSupport()` lists only `{100, 110}`, and `hasMMABlockScale()` lists only `{120}`. cicc v13.0 does not yet emit `mma.sync.block_scale` for sm_120 (the string `mma.sync` does not exist in the binary), suggesting this instruction support is coming in a future CUDA release.
...
sm_120 extends the existing `mma.sync` instruction family (available since Volta/sm_70) with new `block_scale` qualifiers and MX-format data types. The `mma.sync` instruction itself is not new — what's new is the combination:

```
mma.sync.aligned.kind::mxf8f6f4.block_scale.scale_vec::1X.m16n8k32.row.col.f32.e4m3.e4m3.f32.ue8m0
```

This adds per-block MX-format scaling to the synchronous register-based MMA. It supports FP8 (e4m3, e5m2), FP6 (e3m2, e2m3), and FP4 (e2m1) operand types with ue8m0 scale factors. The tile shape is m16n8k32. The upstream LLVM 22 NVPTX backend confirms this with `hasMMABlockScale()` returning true only for `{120}` and `hasMMASparseBlockScaleF4()` for `{120, 121}`.

**cicc v13.0 does not yet emit `mma.sync.block_scale` for sm_120.** The block-scaled MMA support appears to be coming in a future CUDA release. In cicc v13.0, sm_120 can use the legacy HMMA/IMMA tensor core paths (inherited from Volta through Hopper) but not the new Blackwell-generation tensor instructions in either form (tcgen05 or block_scale).
