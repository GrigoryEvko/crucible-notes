...
> **CORRECTION (TCM-1) —** the page-family name "embedded tcmalloc" is a misnomer that this page corrects in place. tcmalloc is *present* (the API/hook/experiment shim, ~55 KiB, ~110 symbols) but *not embedded as the allocator* — its core is not linked. Earlier reports that said host metadata "routes to tcmalloc's `google_malloc` body" are wrong: the `google_malloc` section contains no `malloc`, only support code; all host allocation routes through `__lcxx_override` → PLT → glibc. The [overview.md](overview.md) host-heap row already states this ("libtpu embeds no jemalloc and no tcmalloc"); this page is its detail.
