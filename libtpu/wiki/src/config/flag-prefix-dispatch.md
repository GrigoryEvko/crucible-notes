...
> **CORRECTION (DISPATCH-1) —** the topic was scoped on the hypothesis of "active-gen gating": that an `xla_jf_*` flag might no-op on Viperfish, or be registered with a gen-specific default. The binary refutes the gating reading. There is no per-generation registration switch and no per-generation *application* filter. All TCE flags — codename-prefixed or not — register unconditionally and are written into their TCE field whenever present on the command line, regardless of the active `TpuVersion`. The per-generation behavior lives entirely in (a) the *default* proto loaded per codename (`DefaultsForVersion`) and (b) which fields each consuming pass reads for the active gen. The dispatch is *simpler* than the brief's obvious reading, and this page is framed on the real mechanism. (CERTAIN — `OverrideTpuCompEnvByCmdLineFlags` and `CreateDefaultTpuCompEnv` take no `TpuVersion`; the reflection loop walks the full descriptor.)
