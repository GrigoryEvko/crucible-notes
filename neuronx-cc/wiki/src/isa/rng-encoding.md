...
- [2.1 The 64-Byte Instruction Bundle & Header Skeleton](instruction-bundle.md) — the shared bundle lifecycle (emplace → zero → header → fields → ISA-check → `fwrite 0x40`) every RNG op runs.
- [2.12 Pool / TensorReduce / Reciprocal / Iota Encoding](pool-reduce-encoding.md) — sibling control-band page; the same `assignAccess<TENSORnD>` operand machinery.
