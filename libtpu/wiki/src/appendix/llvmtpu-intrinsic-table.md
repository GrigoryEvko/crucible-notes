...
The single most important structural fact, and a correction to the naive expectation: **the `llvm.tpu.*` surface is the SparseCore intrinsic ISA, not the TensorCore one.** It contains no MXU/matmul and no XLU/transpose intrinsics — those go through a separate `tpu→LLO` TensorCore ODS path. The `llvm.tpu.*` family is dominated (62%) by the SparseCore embedding stream engine. See the [first correction](#correction-intr-1) below.
