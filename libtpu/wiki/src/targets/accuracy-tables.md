...
> **CORRECTION (ACC-1) —** an earlier draft described `CompareAccuracy` as converting the requested `ulps` to a relative error via a `ulps × 2^-23` multiply (`mulsd` from a `.rodata` constant at `0xa2dfb58`) and FATAL-ing on an unhandled mode. Neither holds in the decompile: `CompareAccuracy` (`0x1d60d7a0`) performs **direct `vucomisd` comparisons** of the `ResultAccuracy` tolerance fields against the entry's top-level bound (`entry+0x40`) — there is no `mulsd`-by-constant and no reference to `0xa2dfb58` anywhere in the four consumer bodies (UNVERIFIED → removed). And `GetImplType` returns an **error `Status`** for an unhandled mode, not a fatal abort. The decision *logic* that is decompile-confirmed (CONFIRMED): both `GetTargetAccuracy` calls; the DEFAULT / HIGHEST / TOLERANCE (`mode == 2`) dispatch; the two missing-variant warnings at `:69`/`:61`; and the `"Unsupported result accuracy"` error return at `:79`.
