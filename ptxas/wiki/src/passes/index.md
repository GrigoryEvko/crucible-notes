...
| **Gate passes (AdvancedPhase)** | 16 conditional hooks |
| **Update passes** | 4 data-structure refresh passes |
| **Report passes** | 4 diagnostic/dump passes |
...
| **Reporting** | Dumps IR, statistics, or memory usage for debugging | 6 |
| **Scheduling** | Instruction scheduling, sync insertion, WAR fixup | 8 |
| **RegAlloc** | Register allocation and related fixups | 5 |
| **Encoding** | Mercury SASS encoding, expansion, microcode generation | 8 |
| **Cleanup** | Post-transformation updates, NOP removal, block layout | 9 |
| **Gate** | Conditional hooks (`AdvancedPhase*`) -- no-op by default | 16 |
...
Sixteen phases are conditional extension points whose `isNoOp()` returns `true` in the default vtable. They exist as insertion points for architecture backends and optimization-level overrides. When a specific SM target or `-O` level requires additional processing at a given pipeline position, the backend overrides the phase's vtable to provide a real `execute()` implementation.
...
Four phases refresh data structures invalidated by preceding transformations:
...
These are lightweight passes that call into the IR's internal consistency maintenance routines. They do not transform the IR -- they only update auxiliary data structures (liveness bitmaps, instruction lists, block layout caches) so that downstream passes see a coherent view.

## Report Passes

Six phases produce diagnostic output. They are no-ops unless specific debug options are enabled (e.g., `--stat=phase-wise`, `DUMPIR`, `--keep`):
...
Phase 131 (`DebuggerBreak`) is a development-only hook that triggers a breakpoint -- it is not a report pass per se, but serves a similar diagnostic purpose.
...
| # | Phase Name | Category | Description | Detail Page |
|---|---|---|---|---|
| 14 | `DoSwitchOptFirst` | Optimization | Optimizes switch statements: jump table generation, case clustering (1st pass) | [Branch & Switch](branch-switch.md) |
| 15 | `OriBranchOpt` | Optimization | Branch folding, unreachable block elimination, conditional branch simplification | [Branch & Switch](branch-switch.md) |
...
| 18 | `OriLoopSimplification` | Optimization | Canonicalizes loops: single entry, single back-edge, preheader insertion | [Loop Passes](loop-passes.md) |
...
| 22 | `OriLoopUnrolling` | Optimization | Unrolls loops based on trip count and register pressure heuristics | [Loop Passes](loop-passes.md) |
| 23 | `GenerateMovPhi` | Lowering | Inserts SSA phi nodes as `MOV.PHI` pseudo-instructions | |
| 24 | `OriPipelining` | Optimization | Software pipelining: overlaps loop iterations to hide latency | [Loop Passes](loop-passes.md) |
| 25 | `StageAndFence` | Lowering | Inserts memory fence and staging instructions for coherence | [Sync & Barriers](sync-barriers.md) |
| 26 | `OriRemoveRedundantBarriers` | Optimization | Eliminates barrier instructions proven redundant by data-flow analysis | [Sync & Barriers](sync-barriers.md) |
...
| 28 | `SinkRemat` | Optimization | Sinks instructions closer to uses and marks candidates for rematerialization | [Rematerialization](rematerialization.md) |
...
| 30 | `DoSwitchOptSecond` | Optimization | Second switch optimization pass after loop/branch transformations | [Branch & Switch](branch-switch.md) |
...
| 38 | `OptimizeNestedCondBranches` | Optimization | Simplifies nested conditional branches into flatter control flow | [Branch & Switch](branch-switch.md) |
...
| 49 | `GvnCse` | Optimization | Global value numbering combined with common subexpression elimination | [Copy Prop & CSE](copy-prop-cse.md) |
...
| 54 | `OriDoRematEarly` | Optimization | Early rematerialization: recomputes cheap values near uses to reduce register pressure | [Rematerialization](rematerialization.md) |
...
| 63 | `OriDoPredication` | Optimization | If-conversion: converts short conditional branches into predicated instructions | [Predication](predication.md) |
...
| 69 | `OriDoRemat` | Optimization | Late rematerialization: recomputes values exposed by predication and fusion | [Rematerialization](rematerialization.md) |
| 70 | `OriPropagateVaryingSecond` | Optimization | Propagates varying annotations (2nd pass, after predication changes control flow) | |
| 71 | `OptimizeSyncInstructions` | Optimization | Eliminates and simplifies synchronization instructions | [Sync & Barriers](sync-barriers.md) |
| 72 | `LateExpandSyncInstructions` | Lowering | Expands sync pseudo-instructions into final hardware sequences | [Sync & Barriers](sync-barriers.md) |
...
| 95 | `SetAfterLegalization` | Cleanup | Sets post-legalization flag on the compilation context | |
...
| 99 | `OriDoSyncronization` | Scheduling | Inserts synchronization instructions (`BAR`, `DEPBAR`, `MEMBAR`) per GPU memory model | [Sync & Barriers](sync-barriers.md) |
| 100 | `ApplyPostSyncronizationWars` | Scheduling | Fixes write-after-read hazards exposed by sync insertion | [Sync & Barriers](sync-barriers.md) |
...
| 110 | `PostSchedule` | Scheduling | Post-scheduling pass: finalizes instruction ordering | [Scheduling](../scheduling/overview.md) |
...
| 115 | `AdvancedScoreboardsAndOpexes` | Gate | Full scoreboard generation: computes 23-bit control word per instruction (-O1+); no-op at -O0 | [Scoreboards](../scheduling/scoreboards.md) |
| 116 | `ProcessO0WaitsAndSBs` | Scheduling | Conservative scoreboard insertion for -O0: maximum stalls, barriers at every hazard | [Scoreboards](../scheduling/scoreboards.md) |
...
| 139 | `ProcessO0WaitsAndSBs` | Scheduling | Conservative scoreboard insertion for `-O0`; inserts maximum wait counts at every hazard | [Scoreboards](../scheduling/scoreboards.md) |
...
| 149--158 | *(see batch 2)* | *(varies)* | 10 remaining late-pipeline phases (CalcRegisterMap, diagnostics, debug hooks, NOP) | |

Phases 139--158 are 20 late-pipeline phases whose vtable pointers range from `off_22BEB80` to `off_22BEE78` (40-byte stride). Despite earlier analysis classifying them as "architecture-specific with names returned by vtable `getName()`," all 20 have names in the same static table at `off_22BD0C0` (the table extends to 159 entries, not 139). The vtable `getName()` at +16 returns 0 for all phases; name resolution goes through the static table indexed by `getIndex()` at +8.
...
- [Register Allocator Architecture](../regalloc/overview.md) -- phases 101, 103, 105, 123, 124, 138
...
- [Mercury Encoder](../codegen/mercury.md) -- phases 113, 117--122, 141--147
