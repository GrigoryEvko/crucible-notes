...
> **NOTE — the 6acc60406 callback stamps only `"execute"`.** The decompiled per-op callback (`@0x136066e0`) sets `sc.sequencer = "execute"` and nothing else; there is no `"access"`-emitting branch in this `gfc` build, and the gfc binary cannot reach a TAC split (no `SparseCoreTacCodecBase` on 6acc60406 — see [getSequencerType](getsequencertype.md)). The per-op Access-vs-Execute rule is owned upstream (LOW confidence here).
