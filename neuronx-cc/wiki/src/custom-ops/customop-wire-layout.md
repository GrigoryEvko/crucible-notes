...
- [11.6 Custom-Op Wire Validators](customop-wire-validators.md) — the full `is_valid_custom_op_*` checker set and the codegen-mode plumbing.
- [11.7 FindCustomOpData Staging & FunctionId Binding](findcustomopdata-staging.md) — how the header's FunctionId byte maps to a `(libfile, function-name)` via `ModuleArtifactInfo`.
