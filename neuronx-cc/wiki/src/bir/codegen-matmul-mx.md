...
> **GOTCHA — FP4 output passes the geometry gate but not the slot allow-list.** `getDst0ValidDtypes` admits only `{8, 9}` (e4m3-x4, e5m2-x4), while the geometry verifier's packed-element test (`v30==2 || (v30-8)<=1`) and the sim kernel both treat the x4 set as `{2,8,9}`, i.e. FP4-e2m1-x4 included. On this 2.24 build FP4 output is therefore geometrically legal but slot-rejected; whether FP4 routes through a different dst valid-set or was simply left un-enabled is not decidable from the binary. [INFERRED]
...
- **FP4 output on QuantizeMX.** The `getDst0ValidDtypes = {8,9}` versus x4-geometry `{2,8,9}` split (§4.3) is a real inconsistency inside this build, and the binary does not say which side is authoritative.
