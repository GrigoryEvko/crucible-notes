...
The tile/lane geometry is sourced from `TpuCoreParts::SequencerCount(cp, 5)` and `SequencerParts(5).vector_isa()`. Sequencer type 5 is the SparseCore tile-execute pool — the 16-instance, `lane_count`-16 `VectorIsa` on v7x. So `SparseCoreTiles = 16`, `SparseCoreLaneCount = 16` for v7x, matching the `chip_parts` decode of the `6acc60406` SC sequencer geometry.

> **NOTE —** the authoritative `TpuSequencerType` ordering recovered from the binary's enum strings is `{0 INVALID, 1 TENSOR_CORE_SEQ, 2 BARNA_CORE_SEQ, 3 BARNA_CORE_ADDR_HANDLER, 4 SPARSE_CORE_SEQ (SCS), 5 SPARSE_CORE_TILE_ACCESS_CORE_SEQ (TAC), 6 SPARSE_CORE_TILE_EXECUTE_CORE_SEQ (TEC)}`. `Init` reads type `5`. Some earlier SC-ISA prose numbered the SC sequencers `{3 SCS, 4 TAC, 5 TEC}`; that numbering is off by one and should defer to this enum.
