...
The wiki uses a fixed vocabulary drawn from three sources: EDG's internal source tree, NVIDIA's CUDA extensions, and IDA Pro / Hex-Rays naming conventions. Definitions for the recurring terms:

### Reverse-engineering anchors (IDA / Hex-Rays)

| Term | Definition |
|---|---|
| `sub_<hex>` | IDA Pro's auto-generated name for a function at address `<hex>`. Every function in the binary is referenced by this address-based identifier because the binary is stripped of symbols. Example: `sub_4F2930` is the internal assert handler at `0x4F2930`. |
| `dword_<hex>` / `qword_<hex>` / `byte_<hex>` / `unk_<hex>` | IDA's auto-generated names for global data at `<hex>`. The prefix encodes width (`byte_` = 1 B, `dword_` = 4 B, `qword_` = 8 B, `unk_` = untyped / unknown width). Used throughout as anchors for flag globals, caches, and bitmap tables. |
| `off_<hex>` | IDA's auto-generated name for a pointer-typed global at `<hex>` (a table of pointers, e.g., the IL kind name table `off_E6DD80`). |
| **Sweep** | A Phase 1 analysis pass over a contiguous address range (typically 128--256 KB), enumerating every function in that range. Sweeps are how the function map was built; the 20-sweep grid covers the full `.text` section. |

### EDG core concepts

| Term | Definition |
|---|---|
| **IL** | EDG's Intermediate Language -- the typed, scope-linked node graph that holds every declaration, type, expression, statement, scope, and template after parsing. See [IL Overview](./il/overview.md) for the 85 entry kinds. |
| **Entity** | A named or anonymous declaration node (variable, routine, type, namespace, template parameter). The execution-space bitfield at offset `+182` lives on the entity node. |
| **Entry kind** (`entry_kind`) | The 1-byte tag (values 0--84) that discriminates IL node types. Sub-kind enums use prefixed forms: `tk_*` (type kinds, 22 values), `enk_*` (expression node kinds, 36 values), `stmk_*` (statement kinds, 26 values), `sck_*` (scope kinds, 9 values), `ck_*` (constant kinds, 16 values), `dik_*` (dynamic init kinds, 9 values). |
| **Region** | An IL arena partition. Region 1 holds file-scope nodes (persistent for the whole TU); regions 2..N hold per-function nodes that are freed after each function body is processed. Allocation is bump-style inside 64 KB blocks. |
| **Walk** | A traversal of the IL tree driven by `il_walk.c`. EDG defines five callback slots (enter-node, exit-node, enter-scope, exit-scope, descend-children); the keep-in-IL predicate, display, and constexpr interpreter are all walks. |
| **Gen-routine** | EDG backend code generation for a single C++ declaration. The naming convention for backend dispatchers in `cp_gen_be.c` is `gen_<thing>` -- `gen_routine_decl` (`sub_47BFD0`) for functions, `gen_template` (`sub_47ECC0`) as the master source-sequence dispatcher, `gen_lambda` (`sub_47B890`) for lambdas, etc. |
| **Name lookup** | EDG's resolution machinery in `lookup.c` + `scope_stk.c` that maps an identifier to its declaring entity by walking the scope stack and applying ADL/qualified-lookup rules. |
| **TU** (Translation unit) | One `.cu` source file as seen by the frontend, plus all transitively included headers. The TU descriptor is a 424-byte struct; `reset_tu_state` (`sub_7A4860`) zeros per-TU globals between compilations. |
| **PCH** | Precompiled header. EDG's serialized parser state, gated by the `pch_*` CLI flags (116--121) and `pch.c`. CUDA builds rarely use PCH but the support code is present. |
| **Wrapup** | EDG's term for the 5-pass IL finalization that follows the parse, implemented in `fe_wrapup.c`. Each pass walks the IL applying a different transformation (template instantiation completion, dependent-name resolution, etc.) before backend emission begins. |

### CUDA / NVIDIA additions

| Term | Definition |
|---|---|
| **Kernel stub** / **device stub** | The host-callable forwarding function emitted into `.int.c` for every `__global__` kernel. Generated by `gen_routine_decl` in two passes; the first emits `static void __device_stub__<name>(...)` with a `cudaLaunchKernel` body, the second emits `void <name>(...) { __wrapper__device_stub_<name>(...); }`. See [Kernel Stub Generation](./cuda/kernel-stubs.md). |
| **Host stub** | Same concept as device stub from the host compiler's point of view -- the host-linkage entry point that nvcc's downstream tools resolve. The `--host-stub-linkage-explicit` and `--static-host-stub` flags control its linkage. |
| **RDC** (Relocatable Device Code) / **sepcomp** | Separate compilation mode, enabled by `--device-c` (flag 77, also written `-rdc=true`). Allows device symbols to be defined in one TU and referenced from another, with the device linker `nvlink` resolving cross-TU references. The opposite is **whole-program mode** (the default). |
| **Fatbin** (fatbinary) | The downstream container produced by `fatbinary` (a sibling nvcc tool) that bundles PTX and SASS for multiple GPU architectures. `cudafe++` does not produce it directly, but emits the registration glue (`__cudaRegisterFatBinary`) and the module ID that the runtime uses to bind a host translation unit to its fatbin. |
| **`.cudafe1.c` / `.cudafe1.cpp`** | The downstream filename `nvcc` gives the `.int.c` output of `cudafe++`. Same file format; the suffix differs by language mode (C++ stays `.cpp`). |
| **Host reference array** | One of six `.nvHR*` ELF sections (`.nvHRKE`/`KI`/`DE`/`DI`/`CE`/`CI`) embedded into the `.int.c` output, listing mangled names of `__global__` kernels and `__device__`/`__constant__` variables. The CUDA runtime reads these at startup to build the host-device symbol binding table. See [Host Reference Arrays](./output/host-reference-arrays.md). |
| **Module ID** | A CRC32-derived string (computed by `make_module_id`, `sub_5AF830`) that uniquely identifies a TU's fatbin and seeds anonymous-namespace mangling. See [Module ID & Registration](./output/module-id.md). |
| **`_NV_ANON_NAMESPACE` / `_GLOBAL__N_<module_id>`** | The macro `cudafe++` emits at the top of `.int.c` to give every anonymous namespace a TU-unique name. `_GLOBAL__N_` is the Itanium-ABI mangling prefix for anonymous namespaces; appending the module ID prevents cross-TU symbol collisions under RDC. |
| **Preamble injection** | The backend's emission of `__nv_dl_wrapper_t`, `__nv_hdl_wrapper_t`, and `__nv_hdl_create_wrapper_t` template definitions into the host stream, triggered by the synthetic type `__nv_lambda_preheader_injection`. Performed by `nv_emit_lambda_preamble` (`sub_6BCC20`). |
| **JIT mode** | Compilation mode emitting PTX only (no SASS) so the driver compiles for the target GPU at run time. Gated by `--cuda-jit-mode` and related flags; see [JIT Mode](./cuda/jit-mode.md). |

### Internal C++ idioms (Hex-Rays output)

| Term | Definition |
|---|---|
| **Dispatcher** | A function that switches on a kind byte (entry kind, token kind, attribute kind) and forwards to one of many handler functions. `sub_47ECC0` is the source-sequence dispatcher; `attribute_display_name` dispatches on attribute kind. |
| **Factory** | A static method that constructs a template-instantiated wrapper -- used here specifically for `__nv_hdl_create_wrapper_t::__nv_hdl_create_wrapper`, which the backend emits at every host-device lambda site to wrap the closure into an `__nv_hdl_wrapper_t`. |
| **Vtable slot** | One pointer-sized entry in a C++ virtual method table. Hex-Rays reconstructs virtual calls as `*(qword*)(vptr + 8*N)(...)`; analysis assigns each slot index to a method by tracing the override matrix (see [Virtual Override Matrix](./reference/virtual-override-matrix.md)). The host-device lambda wrapper deliberately avoids vtables and uses static function pointers instead. |
