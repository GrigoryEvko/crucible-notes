...
Five of the seven blocks (`sm50_7x`, `sm75`, `sm100`, `sm80_8x`, `sm86_89`,
tabulated in `per_sm_handler_dispatch.tsv`) share lead handler `0xEA7440` and
draw their per-opcode emitters from one shared low `.text` region (`0xC693D0+`).
The two remaining blocks are:
...
Blocks 6 and 7 share **zero** per-opcode emitter
...
sm_103 (datacenter Blackwell Ultra)
...
## Files
- `extract_arch_blocks.py` — stdlib-only extractor for all seven blocks.
...
- `new_arch_blocks_sm110_sm120.tsv` — the two new blocks (6 = sm_110,
  7 = sm_120/121).
...
- `per_arch_encoding_opbyte.tsv` — behavioural cross-check: per-target SASS
  primary-opcode-byte histogram for a fixed probe (produced by
  `../ptxas-isel/extract_per_arch_isel.py`).
- `sass_class_presence_by_arch.tsv` — per-arch SASS-ISA class presence matrix
  (1 = class exists for that arch), derived from `nvdisasm`'s decoded ISA
  tables; this directory records only the class-name presence facts.
...
(38/31/35/0). The presence matrix in `sass_class_presence_by_arch.tsv` tabulates
only the TMEM/async-warp subset (`ldtm_`, `sttm_`, `utcatomsws_{cas,fas,op}_`,
`utcbar_flush_`, `utcldsws_`, `utcstsws_`), which is the narrower count.
sm_110 keeps almost the whole family (35); sm_103 drops 8
...
The block extractor run against the binary (`sha256 daba837a…`, `V13.0.88`)
reproduces `arch_blocks.tsv` byte-for-byte; the lead headers, slot counts
(1616/1759/2142/1744/1754/657/682), and the empty handler-set intersections are
confirmed by reading `.rodata` directly (objcopy) and disassembling the
lead/slot handlers (objdump). The SASS-ISA class counts and the entire
`sass_class_presence_by_arch.tsv` (209 classes × 4 arches = 836 cells) match the
`nvdisasm -c` class-name sets.
