...
| 3 | The brewer.py assert sites exist and name brewer | `strings | rg 'instabrew/brewer.py'` present in libBIR, libwalrus, libBIRSimulator (1 each); `false && "Not Implemented"`, `Unknown AluOpType`, `Unknown ActivationFunctionType`, indirection-tensor strings all present | **CONFIRMED.** |
| 4 | 27 brewer-owned field-enums, not all 45 | `nm -DC | rg -c '2string\[abi'` = 45 total; 27 fall in VA `0x4002a0..0x402480` | **CONFIRMED.** (Report stated 44 total; the cp310 image actually has **45** — a +1 discrepancy, likely one enum the report omitted; the 27-in-block figure is exact.) |
