...
|---|---|---|
| `nullsub_45` at `0x680190` | sm_50, sm_52, sm_53, sm_60, sm_61, sm_62, sm_70, sm_72, sm_75 | No-op: phase 110 returns immediately. SM-version gate at the top of `sub_C60640` already excludes most of these, but the sentinel covers any remaining cases where `GetSmVersionIndex` returns 0--1 for a sub-target still constructed with the default vtable. |
| Backend A entry (`sub_A97600`, `PostSchedulePass::runOnFunction`, 42 KB) | sm_80--sm_89 legacy fallback (when Backend C is disabled by knob) | Re-runs the main 3-phase scheduling engine (`sub_688DD0`) in **post-RA mode**: rebuilds the dependency graph against physical-register liveness, recomputes latency budgets using actual register-file banks (so the scheduler sees real read-port conflicts), and re-orders the per-BB instruction list. This is the same `sub_688DD0` engine used pre-RA, configured with a different mode byte. |
...
| `sub_A97600` | 42 KB | `PostSchedulePass::runOnFunction` -- legacy Backend A post-RA scheduler installed at sub-target vtable +0x90 on sm_80--sm_89 fallback paths | HIGH |
